...
Another common pattern is to use application access to give a platform administration team or a group of advanced users early access to [beta](https://www.palantir.com/docs/foundry/platform-overview/development-life-cycle/#beta) applications while restricting their availability to the broader user base.
...
Application access is not a security feature; it only simplifies the frontend user experience for users that do not need to view certain applications. Refer to the [Security documentation](https://www.palantir.com/docs/foundry/security/overview/) for guidance on how to properly permission off functionality.

To view and configure the Application access section in Control Panel, a user needs the **Manage application access** workflow, which is granted by the **User experience administrator** role. Roles are administered in the **[Organization permissions](https://www.palantir.com/docs/foundry/administration/enrollments-and-organizations-permissions/#roles)** tab in Control Panel.

![Image 1: Application access page](https://www.palantir.com/docs/resources/foundry/administration/application-access-overview.png)

## Change requests

Changing an application access configuration will generate a change request in [Approvals](https://www.palantir.com/docs/foundry/approvals/overview/). These can be viewed in the [Approvals inbox in Control Panel](https://www.palantir.com/docs/foundry/administration/control-panel-approvals/) with the request type **Application access change requests**. A historical record of all changes made will be kept.

![Image 2: Application access change request](https://www.palantir.com/docs/resources/foundry/administration/application-access-change-request.png)
...
![Image 3: Application access advanced settings](https://www.palantir.com/docs/resources/foundry/administration/application-access-advanced-settings.png)
...
![Image 4: Application access change policy dialog](https://www.palantir.com/docs/resources/foundry/administration/application-access-change-policy-dialog.png)
...
![Image 5: Application access policy change request](https://www.palantir.com/docs/resources/foundry/administration/application-access-policy-change-request.png)
...
![Image 6: Configure platform access](https://www.palantir.com/docs/resources/foundry/administration/application-access-platform-access.png)
...
The scope of the Foundry platform can be restricted on a per-application basis. Users without access to an application will not be able to discover it from the sidebar or [Application Portal](https://www.palantir.com/docs/foundry/getting-started/orientation-and-nav//#applications-portal). Additionally, they will see a 403 "Permission denied" error message when attempting to access an application through a URL to which they do not have access.

![Image 7: Permission denied](https://www.palantir.com/docs/resources/foundry/administration/403-permission-denied.png)

All applications are grouped by category and [lifecycle stage](https://www.palantir.com/docs/foundry/platform-overview/development-life-cycle/), and sorted alphabetically.

![Image 8: Category of applications](https://www.palantir.com/docs/resources/foundry/administration/application-access-category.png)
...
![Image 9: Select multiple applications to manage](https://www.palantir.com/docs/resources/foundry/administration/application-access-bulk-select.png)

In this case, the manage dialog shows all selected applications with their current lifecycle stage and access setting.

![Image 10: Manage multiple applications](https://www.palantir.com/docs/resources/foundry/administration/application-access-bulk-dialog.png)
...
Applications follow the [development lifecycle](https://www.palantir.com/docs/foundry/platform-overview/development-life-cycle/). When an application transitions from one lifecycle stage to the next, the same set of users maintain access, with the following exceptions:
...
Note that all application lifecycle stages will be announced two weeks in advance in the [Announcements](https://www.palantir.com/docs/foundry/announcements/) section of the documentation. Addresses configured in the **Platform administration**[contact information](https://www.palantir.com/docs/foundry/administration/platform-communications/#configure-contact-information) will also be emailed about these changes.
