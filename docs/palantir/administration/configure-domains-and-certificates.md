...
![Image 1: Domains and certificates settings.](https://www.palantir.com/docs/resources/foundry/administration/domain_management_extension.png)
...
*   Select the **+ Add** button in the certificates table. 
    *   _Optional:_ select the **Populate fields from existing certificate** checkbox to copy details from an existing certificate.
...
![Image 2: In-platform steps for downloading CSR, obtaining the signed certificate, and uploading back to Foundry.](https://www.palantir.com/docs/resources/foundry/administration/download-csr.png)

### 3. Upload the signed certificate

*   If the criteria in step 2 are met, upload the signed certificate to the form.
*   The form will run validation checks on the signed certificate. If there are any issues, an error message will appear. Refer to [common errors](https://www.palantir.com/docs/foundry/administration/configure-domains-and-certificates/#common-errors) for guidance.
...
![Image 3: Settings for configuring the domain after signed certificate upload](https://www.palantir.com/docs/resources/foundry/administration/configure-domain.png)
...
![Image 4: DNS settings.](https://www.palantir.com/docs/resources/foundry/administration/domain-management_dns.png)
...
![Image 5: The dialog for creating a new custom domain using an existing certificate](https://www.palantir.com/docs/resources/foundry/administration/domain_management_create_custom_domain.png)
...
![Image 6: Copying configuration from an existing domain.](https://www.palantir.com/docs/resources/foundry/administration/domain_management_copying_domain.png)

Follow the steps below to migrate from an existing domain:

1.   Follow the steps to [Create a new domain](https://www.palantir.com/docs/foundry/administration/configure-domains-and-certificates/#create-a-new-custom-domain).
...
3.   Complete the steps from [create a new custom domain](https://www.palantir.com/docs/foundry/administration/configure-domains-and-certificates/#create-a-new-custom-domain) to generate a certificate signing request and upload the signed certificate.
...
The process for creating a new custom certificate mirrors that of [creating a new custom domain](https://www.palantir.com/docs/foundry/administration/configure-domains-and-certificates/#create-a-new-custom-domain). If no custom domain corresponding to the new certificate’s common name exists, a new one will be created, and the flow will automatically switch to the creation of a new custom domain.
...
![Image 7: Edit active certificate](https://www.palantir.com/docs/resources/foundry/administration/domain_management_edit_active_certificate.png)
...
![Image 8: Delete a domain.](https://www.palantir.com/docs/resources/foundry/administration/domain_management_delete_domain.png)
...
No. Certificates uploaded to Control Panel must be signed by a root CA approved by the Palantir security team to be accepted. Leaf certificates are not permitted on their own. As mentioned in [step 2](https://www.palantir.com/docs/foundry/administration/configure-domains-and-certificates/#2-sign-the-certificate), you must concatenate leaf and intermediate certificates into a single `.pem` file with the leaf certificate first, followed by intermediate(s):
