...
URL Source: https://www.palantir.com/docs/foundry/data-connection/export-overview/
...
Data Connection supports exporting [datasets](https://www.palantir.com/docs/foundry/data-integration/datasets/) and [streams](https://www.palantir.com/docs/foundry/data-integration/streams/) from Foundry to external systems. This can be useful for a variety of purposes:
...
*   Inference results from machine learning models created using [batch deployments](https://www.palantir.com/docs/foundry/model-integration/objectives/#batch-deployments) can be exported to other systems to enable operationalizing ML projects across the organization.
*   Operational data captured from end users in the [Ontology](https://www.palantir.com/docs/foundry/ontology/why-ontology/) can be written to other systems for analysis outside of Foundry.
...
| Export type | Summary |
| --- | --- |
| [File export](https://www.palantir.com/docs/foundry/data-connection/export-overview/#file-exports) | Used for exporting to systems without a schema. Raw files from a dataset are copied on a schedule and written to the target system. |
| [Streaming export](https://www.palantir.com/docs/foundry/data-connection/export-overview/#streaming-exports) | Used for exporting continuously to a system that supports event streaming. Records from a Foundry stream are pushed to the target system continously. |
| [Table export](https://www.palantir.com/docs/foundry/data-connection/export-overview/#table-exports) | Used for exporting to systems with a schema. Rows of a dataset are inserted on a schedule to the target system according to the selected [export mode](https://www.palantir.com/docs/foundry/data-connection/export-overview/#table-export-modes). |

### File exports

An example of a system supporting file exports is [S3](https://www.palantir.com/docs/foundry/available-connectors/amazon-s3/).
...
| Export incrementally | Exports _only_ unexported transactions from the current view without truncating the target table. This mode does not support `UPDATE` and `DELETE` transactions in the dataset being exported. This option mirrors the dataset with `APPEND` transactions only to avoid exporting the same data again. This mode may produce duplicate records in the target table if the upstream dataset has a `SNAPSHOT` transaction. |
...
| Export incrementally and fail if not `APPEND` | Exports only unexported transactions from the current view, failing if there is a `SNAPSHOT`, `UPDATE`, or `DELETE` transaction (after the first run). This option mirrors the Foundry dataset as long as it only contains `APPEND` transactions. It protects against potentially re-exporting all data in case there is a snapshot in the Foundry dataset and guarantees there will never be duplicate data exported to the target table. |

Export modes are not available for legacy export tasks.

![Image 1: On the configuration page users see a form that allows them to input the names for the destination table, schema, and dataset](https://www.palantir.com/docs/resources/foundry/data-connection/table-exports-full-screen.png)
...
*   When using an [agent worker](https://www.palantir.com/docs/foundry/data-connection/set-up-agent/) with table exports, the agent must be running on a Linux host. Windows agents are not supported for running table exports.
...
Table exports are currently supported for custom JDBC source types. Though Foundry provides dedicated connectors for specific systems such as Oracle, the table exports source capability will not appear as an option unless the connection is established via the [custom JDBC connection](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#custom-jdbc-sources).
...
Below, you can find examples of the INSERT statements used by Palantir for exports to [custom JDBC sources](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#custom-jdbc-sources). These statements cannot currently be customized. If your system does not accept the statements used when running exports, you can fall back on code-based connectivity options to write to your system.
...
If a more specific dialect is needed, we recommend one of Foundry's [code-based connectivity options](https://www.palantir.com/docs/foundry/data-connection/core-concepts/#use-in-code) such as [external transforms](https://www.palantir.com/docs/foundry/data-connection/external-transforms/).
...
Foundry datasets containing structured data store their underlying files under a `spark/` directory by default. If you need files to land in a specific directory structure when exported - for example, organized by date partition - you can use a [Python transform](https://www.palantir.com/docs/foundry/transforms-python/overview/) to write the dataset with the desired layout before exporting.

The following Python transform uses the [filesystem API](https://www.palantir.com/docs/foundry/transforms-python/unstructured-files/) to copy files directly to a date-based path:
...
Exportable markings also control which resource metadata can appear in [Slack notifications from monitoring views](https://www.palantir.com/docs/foundry/monitoring-views/external-systems/#configure-exportable-markings-for-resource-name-visibility).

![Image 2: enable exports for source](https://www.palantir.com/docs/resources/foundry/data-connection/exports-enable-and-configure-exportable-markings.png)
...
![Image 3: An empty table on a source export page with a blue Create Export button.](https://www.palantir.com/docs/resources/foundry/data-connection/exports-overview-no-exports-created.png)

Select **Create export**, then select the dataset or stream to export and any source-specific export configuration options. These options will vary by source connector and are explained on the corresponding [source type](https://www.palantir.com/docs/foundry/data-integration/source-type-overview/) pages in our documentation. If multiple branches exist on the exported dataset, only data on the master branch will be exported.

The example below shows the export configuration interface for the [S3](https://www.palantir.com/docs/foundry/available-connectors/amazon-s3/) connector:

![Image 4: The S3 source export configuration page. Available configurations include the dataset and destination for export, the export name, and source-specific settings.](https://www.palantir.com/docs/resources/foundry/data-connection/exports-new-s3.png)
...
Table exports are intended to fully replace the now sunsetted [export tasks](https://www.palantir.com/docs/foundry/data-connection/export-tasks/) that will eventually be deprecated according to the published [product lifecycle](https://www.palantir.com/docs/foundry/platform-overview/development-life-cycle/#sunset). While much of the functionality offered by export tasks is available in the new export options, there are some differences in the features and configurations.
...
| `preSql: <sql statements>` | Not supported | If this is needed for your use case, use [external transforms](https://www.palantir.com/docs/foundry/data-connection/external-transforms/). |
| `stagingSql: <sql statements>` | Not supported | If this is needed for your use case, use [external transforms](https://www.palantir.com/docs/foundry/data-connection/external-transforms/). |
| `afterSql: <sql statements>` | Not supported | If this is needed for your use case, use [external transforms](https://www.palantir.com/docs/foundry/data-connection/external-transforms/). |
| `manualTransactionManagement: <boolean>` | Not supported | If this is needed for your use case, use [external transforms](https://www.palantir.com/docs/foundry/data-connection/external-transforms/). |
...
| `table:` `database: mydb # Optional` `schema: public # Optional` `table: mytable` | Supported | These options are specific to [table exports](https://www.palantir.com/docs/foundry/data-connection/export-overview/#table-exports) on the [JDBC source](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#export-data-to-jdbc-sources). |
...
| `batchSize: <integer>` | Depends on the source type | Batch size is supported for exports on the [JDBC source](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#export-configuration-options-for-jdbc-sources) but may not be supported for other source types. |
| `writeMode: <ErrorIfExists|Append|Overwrite|AppendIfPossible>` `incrementalType: <snapshot|incremental>` | Supported | Together, these options are similar to the [table export modes](https://www.palantir.com/docs/foundry/data-connection/export-overview/#table-export-modes) documented above. Not all combinations are valid for export tasks, and the combinations have been [normalized ↗](https://en.wikipedia.org/wiki/Database_normalization) into the six modes offered for table exports. |
| `exporterThreads: <integer>` | Not supported | If you need granular control over exporter threads, use [external transforms](https://www.palantir.com/docs/foundry/data-connection/external-transforms/). |
