...
URL Source: https://www.palantir.com/docs/foundry/data-connection/agent-worker/

Published Time: Thu, 09 Jul 2026 17:47:53 GMT

# [](https://www.palantir.com/docs/foundry/data-connection/agent-worker/#agent-worker-configuration-reference)Agent worker configuration reference

Legacy

Agent worker is in the [legacy](https://www.palantir.com/docs/foundry/platform-overview/development-life-cycle/) phase of development, and no additional development is expected. Full support remains available. We recommend using [Foundry worker](https://www.palantir.com/docs/foundry/data-connection/core-concepts/#foundry-worker) connections when building a new source or updating existing workflows. See [Foundry worker vs. agent worker](https://www.palantir.com/docs/foundry/data-connection/foundry-worker-vs-agent-worker/) for the operational reasons behind this recommendation.

This page describes the configuration options available when using an agent as a [worker](https://www.palantir.com/docs/foundry/data-connection/core-concepts/#agent-worker) and assumes you are already familiar with [data connection agents](https://www.palantir.com/docs/foundry/data-connection/core-concepts/#agents) and their [generic configuration options](https://www.palantir.com/docs/foundry/data-connection/agent-configuration-reference/).

To migrate an existing agent worker source to a Foundry worker, see [Switch source from agent worker to Foundry worker](https://www.palantir.com/docs/foundry/data-connection/foundry-worker-vs-agent-worker/#switch-source-from-agent-worker-to-foundry-worker).

## [](https://www.palantir.com/docs/foundry/data-connection/agent-worker/#set-up-an-agent)Set up an agent

You must [set up an agent](https://www.palantir.com/docs/foundry/data-connection/set-up-agent/) to use the agent worker. When configuring a source, you can select the **Agent worker** option and choose at least one available agent. Capabilities run over that source connection will be executed as jobs on the agent host.

## [](https://www.palantir.com/docs/foundry/data-connection/agent-worker/#capability-execution)Capability execution

When using an agent worker, capabilities are executed by running Java code on the agent host directly. These Java processes may pull data from your systems and push data up to Foundry (as with [batch syncs](https://www.palantir.com/docs/foundry/data-connection/set-up-sync/)), or pull data from Foundry and push to your systems (as with [exports](https://www.palantir.com/docs/foundry/data-connection/export-overview/)), which comes with [known limitations](https://www.palantir.com/docs/foundry/data-connection/agent-worker/#known-limitations).

## [](https://www.palantir.com/docs/foundry/data-connection/agent-worker/#networking)Networking

All networking for agent worker sources is configured on the agent host. Foundry-side egress policies do not apply: the agent host must have firewalls, DNS, and proxy settings configured to reach the source systems directly. See [Configure agent network access](https://www.palantir.com/docs/foundry/data-connection/set-up-agent/#configure-agent-network-access) for the full reference.

## [](https://www.palantir.com/docs/foundry/data-connection/agent-worker/#memory-allocation-and-usage)Memory allocation and usage
...
For information on monitoring memory usage on agents, review [agent metrics and health monitoring.](https://www.palantir.com/docs/foundry/data-connection/agent-configuration-reference/#agent-metrics-and-health-monitoring)

## [](https://www.palantir.com/docs/foundry/data-connection/agent-worker/#load-balancing)Load balancing

When using an agent worker, multiple agents may be assigned to a single source connection. An agent is assigned jobs to execute specific [capabilities](https://www.palantir.com/docs/foundry/data-connection/core-concepts/#capabilities) configured on assigned source connections. Jobs are executed on one of the available agents at the time when the job is started.

Jobs are assigned randomly to one available agent amongst the assigned agents.

**Maximum concurrent syncs** defaults to `16`, and is configurable under **Agent settings**. The **Maximum concurrent syncs** quota is enforced across all [capabilities](https://www.palantir.com/docs/foundry/data-connection/core-concepts/#capabilities) and all assigned sources, meaning that any run of any capability on any source uses up one unit of the available concurrent sync quota. This includes [streaming syncs](https://www.palantir.com/docs/foundry/data-connection/core-concepts/#streaming-syncs), [change data capture syncs](https://www.palantir.com/docs/foundry/data-connection/core-concepts/#change-data-capture-syncs), [exports](https://www.palantir.com/docs/foundry/data-connection/export-overview/), [webhooks](https://www.palantir.com/docs/foundry/data-connection/webhooks-overview/) and legacy data connection [tasks](https://www.palantir.com/docs/foundry/data-connection/core-concepts/#tasks-sunset). If the assigned agent already has the maximum number of concurrent syncs currently running, then new jobs will be queued until enough running capabilities finish.

In general, we do not recommend using multiple agents as a way to load balance a larger workload than could be successfully run on a single agent. The primary intended use of multiple agents is to allow for agents being taken offline for maintenance. For optimal performance and reliability, we recommend that each agent in a multi-agent setup should be able to handle the full set of [capabilities](https://www.palantir.com/docs/foundry/data-connection/core-concepts/#capabilities) configured on the assigned source connection(s).

## [](https://www.palantir.com/docs/foundry/data-connection/agent-worker/#direct-vs-data-proxy-upload-strategy)Direct vs. data proxy upload strategy

Agent workers support two options to specify how data from batch syncs should be uploaded to the Palantir platform:

### [](https://www.palantir.com/docs/foundry/data-connection/agent-worker/#data-proxy-mode)Data proxy mode
...
### [](https://www.palantir.com/docs/foundry/data-connection/agent-worker/#direct-mode-sunset)Direct mode [Sunset]
...
## [](https://www.palantir.com/docs/foundry/data-connection/agent-worker/#custom-jdbc-drivers)Custom JDBC drivers

See the [JDBC (custom) connector documentation](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#agent-worker-connections-legacy) for instructions on adding custom JDBC drivers to an agent.
...
## [](https://www.palantir.com/docs/foundry/data-connection/agent-worker/#credentials)Credentials
...
More information on moving agents between directories and hosts is covered in the [agent configuration reference documentation](https://www.palantir.com/docs/foundry/data-connection/agent-configuration-reference/#move-an-agent-to-a-new-directory), including instructions for retaining encrypted credentials when moving an existing agent directory.

## [](https://www.palantir.com/docs/foundry/data-connection/agent-worker/#certificates)Certificates
...
[Review the documentation on adding additional certificates for connecting to specific source systems.](https://www.palantir.com/docs/foundry/data-connection/agent-configuration-reference/#certificates)

## [](https://www.palantir.com/docs/foundry/data-connection/agent-worker/#add-a-private-key)Add a private key
...
Copied!```bash
1mkdir /home/<username>/security
2cp <bootvisor_root>/var/data/processes/<bootstrapper_dir>/var/conf/keyStore.jks  /home/<username>/security/
```  
...
Copied!```bash
1$ keytool -importkeystore -srckeystore <CUSTOM_KEYSTORE.jks> -destkeystore /home/<username>/security/keyStore.jks
2Importing keystore CUSTOM_KEYSTORE.jks to keyStore.jks...
3Enter destination keystore password: keystore
4Enter source keystore password:
```  
    *   You can verify that the key/keys were added to the copied keystore using the `keytool -list` command:

Copied!```bash
1$ keytool -list -keystore /home/<username>/security/keyStore.jks
2Enter keystore password:
3Keystore type: jks
4Keystore provider: SUN
5
6Your keystore contains 2 entries
7
8<CUSTOM_KEY>>, 15-Dec-2022, PrivateKeyEntry,
9Certificate fingerprint (SHA-256): A5:B5:2F:1B:39:D3:DA:47:8B:6E:6A:DA:72:4B:0B:43:C7:2C:89:CD:0D:9D:03:B2:3F:35:7A:D4:7C:D3:3D:51
10server, 15-Dec-2022, PrivateKeyEntry,
11Certificate fingerprint (SHA-256): DB:82:66:E8:09:43:30:9D:EF:0A:41:63:72:0C:2A:8D:F0:8A:C1:25:F7:89:B1:A3:6E:6F:C6:C5:2C:17:CB:B2
```  
...
Copied!```bash
1$ keytool -keypasswd -alias <CUSTOM_KEY> -new keystore -keystore /home/<username>/security/keyStore.jks
2Enter keystore password:
```  
4.   In [Data Connection](https://www.palantir.com/docs/foundry/data-connection/overview/), navigate to the agent, then open the **Agent settings** tab. In the **Manage Configuration** section, select **Advanced**, choose the **Agent** tab, and update the `keyStore` to point to the newly copied keystore. Then, add `keyStorePassword` and set it to the appropriate value (`keystore`, by default).

Copied!```text
1security:
2    keyStore: /home/<username>/security/keyStore.jks
3    keyStorePassword: keystore
4    trustStore: var/conf/trustStore.jks
5...
```  

![Image 2: Agent keystore advanced configuration.](https://www.palantir.com/docs/resources/foundry/data-connection/agent-advancedsetting-keystore.png)

1.   Finally, choose the **Explorer** tab and update both the`keyStorePath` and `keyStorePassword`. Save the new configuration.

Copied!```text
1security:
2    keyStorePath: /home/<username>/security/keyStore.jks
3    keyStorePassword: keystore
4    trustStorePath: var/conf/trustStore.jks
5...
```  
2.   Restart the agent.
...
## [](https://www.palantir.com/docs/foundry/data-connection/agent-worker/#known-limitations)Known limitations
...
*   **Lack of support for some capabilities:** Some capabilities, such as [use in code](https://www.palantir.com/docs/foundry/data-connection/core-concepts/#use-in-code), [virtual tables](https://www.palantir.com/docs/foundry/data-integration/virtual-tables/), and virtual media, are incompatible with the agent worker, since these require synchronous connections directly from Foundry and cannot run as jobs on the agent.

[← PREVIOUS Agent configuration reference](https://www.palantir.com/docs/foundry/data-connection/agent-configuration-reference/)

[NEXT Agent proxy configuration reference →](https://www.palantir.com/docs/foundry/data-connection/agent-proxy/)
