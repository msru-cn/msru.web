...
URL Source: https://www.palantir.com/docs/foundry/data-integration/streams/

Markdown Content:

# [](https://www.palantir.com/docs/foundry/data-integration/streams/#streams)Streams
...
## [](https://www.palantir.com/docs/foundry/data-integration/streams/#stream-storage)Stream storage

### [](https://www.palantir.com/docs/foundry/data-integration/streams/#hot-buffer)Hot buffer
...
### [](https://www.palantir.com/docs/foundry/data-integration/streams/#cold-buffer)Cold buffer

All data from within a Foundry stream is transferred from the hot buffer to the cold storage every few minutes. We call this process "archiving", and it makes the data available as a standard Foundry dataset. This means that any Foundry application can operate on streaming data, even if it doesn’t process data in real-time from the hot buffer. The [dataset view](https://www.palantir.com/docs/foundry/data-integration/datasets/) of a Foundry stream behaves exactly as a standard Foundry dataset in the platform.

## [](https://www.palantir.com/docs/foundry/data-integration/streams/#stream-processing)Stream processing

### [](https://www.palantir.com/docs/foundry/data-integration/streams/#read-data-from-a-stream)Read data from a stream
...
### [](https://www.palantir.com/docs/foundry/data-integration/streams/#partitions)Partitions
...
### [](https://www.palantir.com/docs/foundry/data-integration/streams/#supported-field-types)Supported field types
...
### [](https://www.palantir.com/docs/foundry/data-integration/streams/#streaming-jobs)Streaming Jobs

All streaming jobs are represented internally as [job graphs](https://www.palantir.com/docs/foundry/data-integration/flink-streaming/#flink-jobs), which provide a visual representation of your streaming pipeline. As data is processed, it flows through the job graph according to the directed edges until reaching a data sink.

### [](https://www.palantir.com/docs/foundry/data-integration/streams/#checkpointing)Checkpointing
...
## [](https://www.palantir.com/docs/foundry/data-integration/streams/#streaming-consistency-guarantees)Streaming consistency guarantees

Streaming in Foundry operates with two consistency guarantees: `AT_LEAST_ONCE` and `EXACTLY_ONCE`.

### [](https://www.palantir.com/docs/foundry/data-integration/streams/#at_least_once-semantics)AT_LEAST_ONCE semantics
...
#### [](https://www.palantir.com/docs/foundry/data-integration/streams/#benefits-of-at_least_once-semantics)Benefits of `AT_LEAST_ONCE` semantics
...
#### [](https://www.palantir.com/docs/foundry/data-integration/streams/#drawbacks-of-at_least_once-semantics)Drawbacks of `AT_LEAST_ONCE` semantics

*   Requires the downstream consuming application to be able to handle duplicate messages.

### [](https://www.palantir.com/docs/foundry/data-integration/streams/#exactly_once-semantics)`EXACTLY_ONCE` semantics
...
#### [](https://www.palantir.com/docs/foundry/data-integration/streams/#benefits-of-exactly_once-semantics)Benefits of `EXACTLY_ONCE` semantics
...
#### [](https://www.palantir.com/docs/foundry/data-integration/streams/#drawbacks-of-exactly_once-semantics)Drawbacks of `EXACTLY_ONCE` semantics
...
### [](https://www.palantir.com/docs/foundry/data-integration/streams/#latency-trade-offs)Latency trade-offs
...
Streaming sources in Foundry currently only support `AT_LEAST_ONCE` semantics for extracts and exports. Streaming pipelines _do_ support both `AT_LEAST_ONCE` and `EXACTLY_ONCE` semantics, and this is configurable under the **Build settings** section of Pipeline Builder.

![Image 2: Configuring streaming semantics](https://www.palantir.com/docs/resources/foundry/data-integration/configuring-streaming-semantics.png)

[← PREVIOUS Datasets](https://www.palantir.com/docs/foundry/data-integration/datasets/)

[NEXT Media sets (unstructured data) →](https://www.palantir.com/docs/foundry/data-integration/media-sets/)
