...
URL Source: https://www.palantir.com/docs/foundry/building-pipelines/branching-release-process/

Markdown Content:

# [](https://www.palantir.com/docs/foundry/building-pipelines/branching-release-process/#branching-and-release-process)Branching and release process
...
## [](https://www.palantir.com/docs/foundry/building-pipelines/branching-release-process/#scope-of-release)Scope of release

Before we jump into the internals of the release management process, we need to better define exactly "what is the product" that we are releasing? For our purposes we'll define the `product` to be _the set of assets released as a conceptual unit_.

Taking [Recommended Project structure](https://www.palantir.com/docs/foundry/building-pipelines/recommended-project-structure/) as a reference for how we build pipelines in Foundry, we see that the pipeline is defined by multiple projects of different types - datasource, transforms, Ontology and workflow. In most cases, however, we will not manage each project on its own as a product, but instead, each project will define a subset of its resources as one of the following product types:
...
![Image 11: pipeline-overview](https://www.palantir.com/docs/resources/foundry/building-pipelines/pipeline_overview.png)
...
## [](https://www.palantir.com/docs/foundry/building-pipelines/branching-release-process/#product-branch-management)Product branch management

The following section refers to the branching strategy we want to deploy to manage the release of every product. [Learn more about branching in general and how branches work between Code Repositories and datasets.](https://www.palantir.com/docs/foundry/data-integration/branching/)
...
### [](https://www.palantir.com/docs/foundry/building-pipelines/branching-release-process/#common-branches)Common branches

`master`- this is the production branch. As such, it is a protected branch and only the release manager role can approve merging a PR to this branch. It is assumed that the `master` branch is sourced with production data.

`dev` - the `dev` (aka development) branch is a staging branch, derived from the `master`branch. It is mostly relevant when you want to test a complete feature using testing data. The `dev` branch can be sourced from `master` (this happens automatically through [fallback branches](https://www.palantir.com/docs/foundry/code-repositories/branch-settings/#fallback-branches)) or from a UAT data source.

`feature-[X]`- feature branch is where a given feature is being developed and tested. This branch is derived from `dev`branch. This is a short-lived branch, meaning it can be deleted once the branch is being merged into master. Since the branch is derived from `dev`it will be sourced with the same data `dev` branch has.Note that this remains true as long as the fallback branches don't get reconfigured, and as long as the input datasets don't exist on the feature branch.

![Image 12: basic-branching](https://www.palantir.com/docs/resources/foundry/building-pipelines/branches-1.png)

### [](https://www.palantir.com/docs/foundry/building-pipelines/branching-release-process/#other-special-branches)Other special branches

`major-release-[X]`- special branch which integrates all required changes due to changes in the source system's schema (occurring at a specific cadence).

`hot-fix-[X]`- special branch used to fix an issue discovered in production while the code base has moved to a new version. Since we assume dev and master are in sync (unless there is a feature which is being actively tested) it is unlikely we need such a branch.

### [](https://www.palantir.com/docs/foundry/building-pipelines/branching-release-process/#branch-management-workflow)Branch management workflow
...
![Image 13: advanced-branching](https://www.palantir.com/docs/resources/foundry/building-pipelines/branches-2.png)

*   At _t0_,`master`and`dev`branches are created.

*   At _t1_, `feature x`branch is created from `dev`

*   At _t2_, `feature y`branch is created from `dev`

*   At _t3_, `feature x`is merged into `dev`

*   At _t4_, `dev`is merged into `master`,and `feature x`branch is deleted

*   At _t5_, `dev`is merged into`feature y`to pick up the latest state of `dev`

*   At _t6_, feature y is merged into `dev`and later from `dev`to `master`as before
...
### [](https://www.palantir.com/docs/foundry/building-pipelines/branching-release-process/#repository-upgrades)Repository upgrades

_Repository upgrades_ are periodic prompts to update the configuration files in a Code Repository. These updates apply fixes or bump versions of language-bundle dependencies and best practice is to stay on top of applying these recommended upgrades.

![Image 14: upgrade button](https://www.palantir.com/docs/resources/foundry/building-pipelines/upgrade.png)
...
![Image 15: upgrade button](https://www.palantir.com/docs/resources/foundry/building-pipelines/upgrade-2.png)
...
2.   When prompted to upgrade `dev`, an automated feature branch will be created. You can test the configuration changes by running a build on this branch once CI checks are complete. After confirming the transforms still execute after the upgrade, merge the configuration changes into `dev`. Once `dev`is upgraded, a PR should be used to push these changes to master.

3.   To upgrade all open feature branches, merge `dev`back into each branch to include these configuration upgrades.

### [](https://www.palantir.com/docs/foundry/building-pipelines/branching-release-process/#maintenance-windows)Maintenance windows
...
![Image 16: upgrade-windows](https://www.palantir.com/docs/resources/foundry/building-pipelines/branches-3.png)
...
## [](https://www.palantir.com/docs/foundry/building-pipelines/branching-release-process/#feature-testing-and-end-user-verification)Feature testing and end user verification
...
The developer-level testing can be done using dataset [Health Checks](https://www.palantir.com/docs/foundry/health-checks/overview/), dependent test datasets, or manually using tools such as [Contour](https://www.palantir.com/docs/foundry/contour/overview/).
...
![Image 17: pull-request](https://www.palantir.com/docs/resources/foundry/building-pipelines/branches-4.png)
...
To review schema changes in a pull request, use schema diff view in the [Affected Datasets](https://www.palantir.com/docs/foundry/code-repositories/analyze-impact/) tab.

1.   The dev-lead runs the build on the `dev`branch to create the feature output datasets.
2.   The dev creates a Foundry Issue on the output dataset, documents any testing and verification requests and assigns the issue to the relevant end user for verification.
3.   The end user receives the issue, with full context and view of the relevant dataset and branch to be tested. They collaborate with the developer on the required changes until the issue is resolved and marked as ready to merge. The issue is then assigned to the release manager.
4.   The release manager closes the issue and merges `dev`into `master`.
5.   The release manager deletes the feature branch.
...
### [](https://www.palantir.com/docs/foundry/building-pipelines/branching-release-process/#foundry-issues-feature-review-workflow)Foundry Issues feature review workflow
...
![Image 18: issue-review](https://www.palantir.com/docs/resources/foundry/building-pipelines/branches-5.png)
...
![Image 19: issue-review3](https://www.palantir.com/docs/resources/foundry/building-pipelines/branches-7.png)
...
![Image 20: issue-review4](https://www.palantir.com/docs/resources/foundry/building-pipelines/branches-8.png)

[← PREVIOUS Development best practices](https://www.palantir.com/docs/foundry/building-pipelines/development-best-practices/)

[NEXT Scheduling best practices →](https://www.palantir.com/docs/foundry/building-pipelines/scheduling-best-practices/)
