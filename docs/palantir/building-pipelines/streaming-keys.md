...
URL Source: https://www.palantir.com/docs/foundry/building-pipelines/streaming-keys/

Published Time: Thu, 09 Jul 2026 17:47:52 GMT

# [](https://www.palantir.com/docs/foundry/building-pipelines/streaming-keys/#streaming-keys)Streaming keys
...
## [](https://www.palantir.com/docs/foundry/building-pipelines/streaming-keys/#partition-keys)Partition keys

Generally, [_primary_ keys](https://www.palantir.com/docs/foundry/transforms-python/data-expectations-reference/#primary-key) in the Palantir platform are used to uniquely identify a database record. However, _partition_ keys in Foundry streams group _records_ with the same **key**, such as all _readings_ for a particular **device**, or all _transactions_ for a particular **customer**. In contrast to primary keys in batch pipelines, partition keys in streams do not deduplicate records since they do not uniquely identify records.
...
| Key | Value | isDeleted |
 ...
 | Key2 | `null` | `false` | | Key1 | thirdVal | `true` | | Key2 | secondVal | `false` | | Key1 | firstVal | `false` |
...
## [](https://www.palantir.com/docs/foundry/building-pipelines/streaming-keys/#key-propagation)Key propagation
...
## [](https://www.palantir.com/docs/foundry/building-pipelines/streaming-keys/#use-a-streaming-key)Use a streaming key
...
## [](https://www.palantir.com/docs/foundry/building-pipelines/streaming-keys/#check-current-keys)Check current keys

The following sections describe how to find and verify streaming key logic in your Foundry stream.

### [](https://www.palantir.com/docs/foundry/building-pipelines/streaming-keys/#check-partition-keys)Check partition keys
...
If a stream already has one or more partition keys, adding a new partition key column will cause a _weaker_ ordering guarantee since there will be more partitions; order is only guaranteed to be maintained for rows that share the same value for all partition key columns.
...
### [](https://www.palantir.com/docs/foundry/building-pipelines/streaming-keys/#check-primary-keys)Check primary keys
...
## [](https://www.palantir.com/docs/foundry/building-pipelines/streaming-keys/#streaming-key-best-practices)Streaming key best practices
...
[← PREVIOUS Streaming compute usage](https://www.palantir.com/docs/foundry/building-pipelines/streaming-compute-usage/)

[NEXT Streaming stateful transforms →](https://www.palantir.com/docs/foundry/building-pipelines/streaming-stateful-transforms/)
