...
URL Source: https://www.palantir.com/docs/foundry/building-pipelines/maintaining-incremental-performance/

Published Time: Thu, 09 Jul 2026 17:47:52 GMT

# [](https://www.palantir.com/docs/foundry/building-pipelines/maintaining-incremental-performance/#maintaining-high-performance)Maintaining high performance
...
## [](https://www.palantir.com/docs/foundry/building-pipelines/maintaining-incremental-performance/#possible-causes)Possible causes

### [](https://www.palantir.com/docs/foundry/building-pipelines/maintaining-incremental-performance/#large-file-counts)Large file counts
...
Additionally, as part of writing a Spark DataFrame to an output in an APPEND or UPDATE transaction, an incremental transform needs to construct an in-memory list of all of the existing files in the output's current view. For outputs with many files, this operation can take a long time and may require the use of a high driver memory [Spark profile](https://www.palantir.com/docs/foundry/optimizing-pipelines/spark-concepts/#tuning-spark-profiles) to avoid out-of-memory errors.

### [](https://www.palantir.com/docs/foundry/building-pipelines/maintaining-incremental-performance/#large-transaction-counts)Large transaction counts

Similarly to having too many files, too many [transactions](https://www.palantir.com/docs/foundry/data-integration/datasets/#transactions) in the view can cause slowness. However, this slowness occurs not only when files are requested, but whenever the view range is resolved, which can affect dataset functionality such as computing stats, loading history, and so on. In transforms, this slowness will manifest in the pre-compute stage, when the environment is being set up and before any Spark details are available, making the issue difficult to debug.
...
## [](https://www.palantir.com/docs/foundry/building-pipelines/maintaining-incremental-performance/#possible-solutions)Possible solutions

### [](https://www.palantir.com/docs/foundry/building-pipelines/maintaining-incremental-performance/#regular-snapshotting)Regular snapshotting
...
### [](https://www.palantir.com/docs/foundry/building-pipelines/maintaining-incremental-performance/#retention-policies)Retention policies
...
## [](https://www.palantir.com/docs/foundry/building-pipelines/maintaining-incremental-performance/#additional-options-to-consider)Additional options to consider

### [](https://www.palantir.com/docs/foundry/building-pipelines/maintaining-incremental-performance/#aborting-transactions)Aborting transactions
...
### [](https://www.palantir.com/docs/foundry/building-pipelines/maintaining-incremental-performance/#changelog-datasets)Changelog datasets
...
[← PREVIOUS Creating incremental syncs](https://www.palantir.com/docs/foundry/building-pipelines/create-incremental-syncs/)

[NEXT Streaming pipelines / Overview →](https://www.palantir.com/docs/foundry/building-pipelines/streaming-overview/)
