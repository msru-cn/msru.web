...
URL Source: https://www.palantir.com/docs/foundry/building-pipelines/scheduling-best-practices/

Published Time: Thu, 09 Jul 2026 17:47:52 GMT

# [](https://www.palantir.com/docs/foundry/building-pipelines/scheduling-best-practices/#scheduling-best-practices)Scheduling best practices
...
*   Trashed datasets should not be built by your schedule. Use the [Upgrade Assistant](https://www.palantir.com/docs/foundry/upgrade-assistant/platform-changes/) to investigate and remedy your schedules that contain trashed resources.
...
## [](https://www.palantir.com/docs/foundry/building-pipelines/scheduling-best-practices/#examples)Examples

A pipeline with unnecessary schedules that should be combined into one schedule:

![Image 5: Unnecessary schedule in one pipeline](https://www.palantir.com/docs/resources/foundry/building-pipelines/too_many_schedules.png)

Combine datasets into a logical schedule using a connecting build:

![Image 6: Use a connecting build](https://www.palantir.com/docs/resources/foundry/building-pipelines/one_connecting_build.png)

Use the guidance below to set up an effective system of schedules for your pipelines.

## [](https://www.palantir.com/docs/foundry/building-pipelines/scheduling-best-practices/#best-practices)Best practices

### [](https://www.palantir.com/docs/foundry/building-pipelines/scheduling-best-practices/#one-schedule-per-pipeline)One schedule per pipeline
...
3.   Refer to [stability recommendations](https://www.palantir.com/docs/foundry/maintaining-pipelines/stability-recommendations/) for more information on how to effectively handle complex pipelines like these.

![Image 7: Pipeline inputs and outputs](https://www.palantir.com/docs/resources/foundry/building-pipelines/pipeline_io.png)

### [](https://www.palantir.com/docs/foundry/building-pipelines/scheduling-best-practices/#project-scoped-schedules)Project-scoped schedules
...
### [](https://www.palantir.com/docs/foundry/building-pipelines/scheduling-best-practices/#one-owner-per-schedule-for-user-scoped-schedules)One owner per schedule for user-scoped schedules
...
### [](https://www.palantir.com/docs/foundry/building-pipelines/scheduling-best-practices/#using-retries)Using retries
...
There are some cases where you might not want to enable retries on your job. For example, pipelines with very tight Service Level Agreements (SLAs) where you want to be alerted as soon as there’s a failure, or transforms that have a non-idempotent side effect. However, note that even if you do not enable retries in your schedule configuration, jobs may still be retried as part of [adjudication](https://www.palantir.com/docs/foundry/transforms-common/transforms-versions/#runtime-versioning).

### [](https://www.palantir.com/docs/foundry/building-pipelines/scheduling-best-practices/#no-multi-dataset-force-builds)No multi-dataset force builds
...
### [](https://www.palantir.com/docs/foundry/building-pipelines/scheduling-best-practices/#what-to-include-and-what-to-exclude-in-a-schedule)What to include and what to exclude in a schedule
...
![Image 8: Pipeline inputs and outputs](https://www.palantir.com/docs/resources/foundry/building-pipelines/pipeline_io.png)

In addition, schedules should not depend on or try to build datasets that are:

*   **Currently in the trash:** To remediate existing schedules with trashed resources, use the [Upgrade Assistant](https://www.palantir.com/docs/foundry/upgrade-assistant/platform-changes/) to investigate the schedules you own to either un-trash or exclude the resource(s) in question from the schedule.
...
### [](https://www.palantir.com/docs/foundry/building-pipelines/scheduling-best-practices/#fail-fast)Fail fast
...
### [](https://www.palantir.com/docs/foundry/building-pipelines/scheduling-best-practices/#name-schedules)Name schedules
...
[← PREVIOUS Branching and release process](https://www.palantir.com/docs/foundry/building-pipelines/branching-release-process/)

[NEXT Building a production pipeline →](https://www.palantir.com/docs/foundry/building-pipelines/building-production-pipeline/)
