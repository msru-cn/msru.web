...
URL Source: https://www.palantir.com/docs/foundry/building-pipelines/remove-markings/

Markdown Content:

# [](https://www.palantir.com/docs/foundry/building-pipelines/remove-markings/#guidance-on-removing-markings)Guidance on removing markings

Access requirements for platform resources are controlled by [Markings](https://www.palantir.com/docs/foundry/security/markings/). Markings restrict access in an **all-or-nothing** fashion: in order to access a resource, a user must be a member of all Markings applied to the resource. In addition, Markings are **inherited** along both the file hierarchy and direct dependencies. If you have the correct permissions, you can remove Markings directly from resources and [along direct dependencies](https://www.palantir.com/docs/foundry/building-pipelines/remove-inherited-markings/).
...
*   If you are starting to apply Markings to your pipelines, we suggest reading through the [scenarios](https://www.palantir.com/docs/foundry/building-pipelines/remove-markings/#scenarios) section below to make sure you don't accidentally lock out any of your end users.
*   Additionally, we recommend following the [best practices](https://www.palantir.com/docs/foundry/building-pipelines/remove-markings/#best-practices) when applying and removing Markings, and consulting the [documentation on how to remove inherited Markings and Organizations](https://www.palantir.com/docs/foundry/building-pipelines/remove-inherited-markings/) for more advanced information.

You can also [remove markings or organizations with Pipeline Builder](https://www.palantir.com/docs/foundry/pipeline-builder/outputs-remove-markings-and-organizations/).

## [](https://www.palantir.com/docs/foundry/building-pipelines/remove-markings/#scenarios)Scenarios

Below are three scenarios related to applying a Marking early in a pipeline and removing it later in a pipeline.

### [](https://www.palantir.com/docs/foundry/building-pipelines/remove-markings/#scenario-1-replacing-severing-by-marking-removal)Scenario 1: Replacing severing by Marking removal
...
![Image 7: scenario1](https://www.palantir.com/docs/resources/foundry/building-pipelines/scenario1.png?width=600)
In the old state shown in the example above, severing has been used to prevent the Marking from propagating. Assuming that severing is only being used to remove a Marking, we strongly recommend that you replace severing with [Marking removal](https://www.palantir.com/docs/foundry/building-pipelines/remove-inherited-markings/#input-transform-property), as in the new state in the example above. When removing the Marking, it is useful to think about the [approval mode](https://www.palantir.com/docs/foundry/building-pipelines/remove-inherited-markings/#approval-modes) configuration of the repository which contains the Marking removal transform.

In the case that propagate view requirements is enabled, read scenario 2 below.

### [](https://www.palantir.com/docs/foundry/building-pipelines/remove-markings/#scenario-2-apply-a-marking-followed-by-marking-removal-to-disable-propagate-view-requirements)Scenario 2: Apply a Marking followed by Marking removal to disable "Propagate View Requirements"
...
![Image 8: propagate_view_requirement_off](https://www.palantir.com/docs/resources/foundry/building-pipelines/propagate_view_requirement_off.png?width=300)
...
![Image 9: propagate_view_requirement_on](https://www.palantir.com/docs/resources/foundry/building-pipelines/propagate_view_requirement_on.png?width=300)
...
We highly recommend _**disabling**_ _view requirement propagation_ in favor of using _Markings_.
...
![Image 10: scenario2](https://www.palantir.com/docs/resources/foundry/building-pipelines/scenario2.png?width=600)
...
This change allows disabling of "propagate view requirements" by using a security Marking. In the proposed solution below, we’ll apply a Marking on `raw_dataset_1` which is then _immediately_ propagated to all downstream datasets which have any non-severed transaction of `raw_dataset_1` as an input. There is an assumption that **severing was already in place**, and severing is only being replaced by Marking removal. If this is not true in your situation, see [Scenario 3](https://www.palantir.com/docs/foundry/building-pipelines/remove-markings/#scenario-3-applying-a-new-marking-followed-by-marking-removal-at-the-dataset-level), where we discuss implications of applying a Marking to a dataset in detail.
...
### [](https://www.palantir.com/docs/foundry/building-pipelines/remove-markings/#scenario-3-applying-a-new-marking-followed-by-marking-removal-at-the-dataset-level)Scenario 3: Applying a new Marking followed by Marking removal at the dataset level
...
![Image 11: scenario3](https://www.palantir.com/docs/resources/foundry/building-pipelines/scenario3.png?width=600)
...
![Image 12: scenario3_2](https://www.palantir.com/docs/resources/foundry/building-pipelines/scenario3_2.png?width=600)
...
## [](https://www.palantir.com/docs/foundry/building-pipelines/remove-markings/#best-practices)Best practices
...
*   **Do not** write Marking removal transforms that filter out data which requires a Marking. This is because new data which merits a Marking can commonly be introduced upstream, and you need to protect against it accidentally flowing through your Marking removal transform. 
    *   Examples showing “filter in” approach (recommended): Copied!```python
1# column-based
2df.select("salary","title","department")
3
4# row-based
5states_to_keep =["OH","CA","DE"]
6df.filter(df.state.isin(states_to_keep))
```  
    *   Examples showing “filter out” approach (**not** recommended): Copied!```python
1# column-based
2df.drop("firstname","lastname")
3
4# row-based
5states_to_drop =["FL","TX","IL"]
6df.filter(~df.state.isin(states_to_drop))
```  
...
*   **Do** remove Marking from the data either as far upstream (when a marked dataset is added as an import) or as far downstream (in the last step before creating a Project export) as possible while performing Marking removal _within_ a Project.

## [](https://www.palantir.com/docs/foundry/building-pipelines/remove-markings/#faqs)FAQs

### [](https://www.palantir.com/docs/foundry/building-pipelines/remove-markings/#do-we-need-an-approval-for-the-marking-removal-every-time-there-is-any-logic-changes-in-the-code)Do we need an approval for the Marking removal every time there is any logic changes in the code?

This depends on whether the **require re-approval** or **don't require re-approval** approval mode is set on the repository which has your Marking removal transform. [Learn more about approval modes.](https://www.palantir.com/docs/foundry/building-pipelines/remove-inherited-markings/#approval-modes)

### [](https://www.palantir.com/docs/foundry/building-pipelines/remove-markings/#the-marking-removal-workflow-and-the-one-repo-per-project-recommendations-dont-go-very-well-together-how-many-repositories-should-we-set-up-per-project-for-the-marking-removal-workflow)The Marking removal workflow and the “one repo per Project” recommendations don’t go very well together. How many repositories should we set up per Project for the Marking removal workflow?

Ideally, every time the _logic_ of a Marking removal transform changes, it should undergo a security approval. To balance between excessive friction in the approval process and good security posture, we recommend that, if you can, move all transforms with Marking removal logic (such as obfuscating data, removing columns, and so on) to a separate repo and set the separate repo to “Require re-approvals”.

### [](https://www.palantir.com/docs/foundry/building-pipelines/remove-markings/#can-i-add-the-marking-removal-properties-stop_propagating-and-stop_requiring-on-an-output-in-my-transforms)Can I add the Marking removal properties (_stop\_propagating_ and _stop\_requiring_) on an output in my transforms?
...
### [](https://www.palantir.com/docs/foundry/building-pipelines/remove-markings/#which-languages-support-marking-removal)Which languages support Marking removal?

The following languages support Marking removal:

*   [Python](https://www.palantir.com/docs/foundry/building-pipelines/remove-inherited-markings/#python)
*   [SQL](https://www.palantir.com/docs/foundry/building-pipelines/remove-inherited-markings/#sql)
*   [Java](https://www.palantir.com/docs/foundry/building-pipelines/remove-inherited-markings/#java)

### [](https://www.palantir.com/docs/foundry/building-pipelines/remove-markings/#why-is-marking-removal-preferred-over-severing)Why is Marking removal preferred over severing?
...
[← PREVIOUS Overview](https://www.palantir.com/docs/foundry/building-pipelines/security-overview/)

[NEXT Remove inherited markings and organizations →](https://www.palantir.com/docs/foundry/building-pipelines/remove-inherited-markings/)
