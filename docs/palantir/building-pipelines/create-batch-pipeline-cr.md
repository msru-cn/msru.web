...
URL Source: https://www.palantir.com/docs/foundry/building-pipelines/create-batch-pipeline-cr/
...
Get started by creating a new repository. To do so, navigate to a [Project](https://www.palantir.com/docs/foundry/getting-started/projects-and-resources/) in Foundry, select **+ New** in the top-right, then select **Code repository**.
...
![Image 1: new-branch-dialog](https://www.palantir.com/docs/resources/foundry/building-pipelines/initialize-repository.png)
...
[`Download titanic.csv`](https://www.palantir.com/docs/resources/foundry/building-pipelines/titanic.csv)

Refer to the guide on [manual data uploads](https://www.palantir.com/docs/foundry/compass/manually-upload-data/) to learn how to upload this dataset into your Project alongside your repository.

![Image 2: repository and dataset](https://www.palantir.com/docs/resources/foundry/building-pipelines/repo-and-dataset.png)

## 3. Create a branch

By creating personal **branches** to make changes, instead of directly editing the master version of the code, you can safely collaborate on the same Code Repository with your colleagues. You can track and undo changes as well as merge changes into the master code. Code can be reviewed on a line-by-line basis meaning that changes to production pipelines can easily be discussed amongst teammates. To learn more about branching in Foundry, see [this page](https://www.palantir.com/docs/foundry/data-integration/branching/).
...
Before you can add changes to your Code Repository, you must first create your own branch which contains a copy of the code on the `master` branch. To create your branch, click the ![Image 3: new-branch](https://www.palantir.com/docs/resources/foundry/building-pipelines/new-branch.png?width=25) icon next to the current branch name.

This opens a dialog for selecting an existing branch and choosing a custom name for the new branch:

![Image 4: new-branch-dialog](https://www.palantir.com/docs/resources/foundry/building-pipelines/new-branch-dialog.png)
...
![Image 5: create-new-file](https://www.palantir.com/docs/resources/foundry/building-pipelines/create-new-file.gif)
...
![Image 6: select dataset](https://www.palantir.com/docs/resources/foundry/building-pipelines/select-dataset.png)
...
![Image 7: verb-autocomplete](https://www.palantir.com/docs/resources/foundry/building-pipelines/verb-autocomplete.png)
...
![Image 8: finished-code](https://www.palantir.com/docs/resources/foundry/building-pipelines/finished-code.png)

Note that you must define an alias for any derived columns you create in SQL. For more information about writing SQL data transformations, refer to the [Spark SQL language reference](https://www.palantir.com/docs/foundry/transforms-sql/spark-reference/).
...
Clicking the **Commit** button commits your changes and runs automatic checks on your code. Clicking the **Build** button also commits your changes. Specifically, clicking **Build** runs automatic code checks and starts building your output dataset. If you want to quickly test your changes without building your dataset to ensure your code passes the code checks, click **Commit**. Otherwise, you can skip ahead to [build your dataset](https://www.palantir.com/docs/foundry/building-pipelines/create-batch-pipeline-cr/#build-your-dataset-on-your-branch).

To commit the changes you’ve made, click the ![Image 9: commit](https://www.palantir.com/docs/resources/foundry/building-pipelines/commit.png?width=80) button at the top right corner and enter a summary of the changes you've made. Committing changes triggers automatic checks to run on your code. An icon in the top-right corner indicates the status of these checks; hover over it to see more details.

![Image 10: check-status](https://www.palantir.com/docs/resources/foundry/building-pipelines/check-status.png)

### Build your dataset on your branch

To test your changes, click the ![Image 11: build-button](https://www.palantir.com/docs/resources/foundry/building-pipelines/build-button.png?width=70) button at the top of the screen.

Once you click the build button, two things happen: automatic checks run on your code and your output dataset starts to build. During this time, either a new output dataset will be created from the code in your branch or an existing output dataset will get updated to reflect your changes. You can view the progress of the running tasks in the **Build** helper. Once the tasks complete, the ![Image 12: checks-passed](https://www.palantir.com/docs/resources/foundry/building-pipelines/checks-passed.png?width=25) icon indicates that each task has successfully completed. If you see the ![Image 13: checks-failed](https://www.palantir.com/docs/resources/foundry/building-pipelines/checks-failed.png?width=25) icon instead, click the **details** button in the **Build** helper to learn more about what went wrong. This will take you to the **Checks** tab where you can look for error messages and also re-trigger your build.
...
![Image 14: preview-dataset](https://www.palantir.com/docs/resources/foundry/building-pipelines/preview-dataset.png)
...
Users with **Owner** permissions will be able to enable the option to “Automatically merge changes” when creating a _pull request_. This option is only available if at least one required check is configured and passing for your repository’s branch. If you enable the option to “Automatically merge changes”, your _pull request_ will automatically get merged into the main code after you create it. Once the changes from your branch are merged into the main code, your branch will also get automatically deleted.

To create a new **Pull Request**, click the ![Image 15: propose-changes](https://www.palantir.com/docs/resources/foundry/building-pipelines/propose-changes.png?width=110) button at the top right corner. This will open the "New pull request" page where you can write a description of your changes and click the **Create pull request** button.

![Image 16: pr-page](https://www.palantir.com/docs/resources/foundry/building-pipelines/pr-page.png)
...
[Learn more about understanding the impact of changes in a Pull Request.](https://www.palantir.com/docs/foundry/code-repositories/analyze-impact/)
...
Once your changes have been reviewed, a user with the appropriate permissions (by default, Owners and Editors) can _merge_ the changes on your pull request to combine them into `master`.
...
Your repository may have different policies that must be met before changes are merged. Policies are defined by the repository owner in the [branch settings page](https://www.palantir.com/docs/foundry/code-repositories/branch-settings/) and presented to code authors in the Pull Request page.
...
Now you can delete the branch you created at the start of this tutorial to reduce clutter. Since your changes have been merged into the `master` branch, there is no need to keep your branch. Navigate to the **Branches** tab, and look under “Personal branches”. Delete the branch you created by clicking the ![Image 17: trash](https://www.palantir.com/docs/resources/foundry/building-pipelines/trash.png?width=25) icon:

![Image 18: delete-branch](https://www.palantir.com/docs/resources/foundry/building-pipelines/delete-branch.png)
...
To delete a protected branch, you first need to [unprotect](https://www.palantir.com/docs/foundry/code-repositories/branch-settings/#unprotect-branches) it and then follow the steps above.

## 8. Build your dataset on the `master` branch

The final step is to build your new dataset on the `master` branch. Similar to [build the dataset in your branch](https://www.palantir.com/docs/foundry/building-pipelines/create-batch-pipeline-cr/#build-your-dataset-on-your-branch), click the ![Image 19: build](https://www.palantir.com/docs/resources/foundry/building-pipelines/build-button.png?width=86) button at the top of the screen when your SQL code file is selected.
...
*   Learn [how to create an incremental pipeline](https://www.palantir.com/docs/foundry/building-pipelines/incremental-overview/).
*   Learn how to create more complex pipelines using [Python Transforms](https://www.palantir.com/docs/foundry/transforms-python/getting-started/).
*   Explore details about core concepts like [datasets](https://www.palantir.com/docs/foundry/data-integration/datasets/), [branching](https://www.palantir.com/docs/foundry/data-integration/branching/), and [builds](https://www.palantir.com/docs/foundry/data-integration/builds/).
...
![Image 20: commit-history](https://www.palantir.com/docs/resources/foundry/building-pipelines/commit-history.png)
