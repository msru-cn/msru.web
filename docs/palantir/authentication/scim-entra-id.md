...
# [](https://www.palantir.com/docs/foundry/authentication/scim-entra-id/#provisioning-with-microsoft-entra-id)Provisioning with Microsoft Entra ID

This section contains steps specific to Entra ID (formerly known as Azure AD) for configuring SCIM.

This step requires coordination with your Entra admin. Additionally, if your SSO uses [SAML](https://www.palantir.com/docs/foundry/authentication/scim-entra-id/#saml), the steps differ slightly from those for [OIDC](https://www.palantir.com/docs/foundry/authentication/scim-entra-id/#oidc).
...
## [](https://www.palantir.com/docs/foundry/authentication/scim-entra-id/#configuration-steps)Configuration steps

### [](https://www.palantir.com/docs/foundry/authentication/scim-entra-id/#saml)SAML

#### [](https://www.palantir.com/docs/foundry/authentication/scim-entra-id/#1-turn-on-scim-provisioning)1. Turn on SCIM provisioning
...
3.   Client ID and secret: generated in [Step 4: Generate SCIM credentials](https://www.palantir.com/docs/foundry/authentication/scim-enable/#4-generate-scim-credentials).

![Image 3: The Entra ID UI for configuring SCIM client credentials.](https://www.palantir.com/docs/resources/foundry/authentication/scim-entra-provisioning-credentials.png)

#### [](https://www.palantir.com/docs/foundry/authentication/scim-entra-id/#2-configure-attribute-mapping)2. Configure attribute mapping
...
![Image 4: The Entra ID UI for configuring attribute mapping for SCIM group provisioning.](https://www.palantir.com/docs/resources/foundry/authentication/scim-entra-group-attribute-mapping.png)

#### [](https://www.palantir.com/docs/foundry/authentication/scim-entra-id/#3-toggle-provisioning-status-to-on)3. Toggle provisioning status to `On`

1.   This will start the initial sync, which will ensure that every user and group assigned to this application exists in Foundry and all group memberships are updated. Foundry will also perform [organization assignment](https://www.palantir.com/docs/foundry/authentication/org-assignment/), [user intake evaluation](https://www.palantir.com/docs/foundry/authentication/intake-forms/), and [rule based group evaluation](https://www.palantir.com/docs/foundry/authentication/group-assignment/) for all rules configured in Control Panel. It will not run asynchronous user managers. All of this information will also refresh on the user's next login.
