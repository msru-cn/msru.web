...
URL Source: https://www.palantir.com/docs/foundry/code-workspaces/rstudio/
...
3  session$allowReconnect("force")
4}
```
...
![Image 1: The blue build button to the right of the dataset name.](https://www.palantir.com/docs/resources/foundry/code-workspaces/create-r-transform.png?width=500)
...
![Image 2: The Edit Transform pane,l with options to modify the script file, inputs, and outputs and adjust the CPU and Memory.](https://www.palantir.com/docs/resources/foundry/code-workspaces/r-transform-configuration.png?width=300)
...
Once CI checks are completed, your transform is ready for build. You can then use other Foundry [data integration](https://www.palantir.com/docs/foundry/data-integration/overview/) tools to manage your transform and the [data pipeline](https://www.palantir.com/docs/foundry/data-integration/data-pipeline/) to which it belongs.

The transform does not automatically use the environment of the Code Workspace. You should install all the libraries needed at runtime at the top of the script file. The code snippet to install a library can be found in the **libraries** panel. For example, to use `dplyr` and `tidyr` in your transform, add this code snippet to the top of the file:

Copied!

`1renv::install(c("dplyr", "tidyr"))`

[Learn more about R transforms.](https://www.palantir.com/docs/foundry/transforms-r/overview/)
...
Reports are built using transforms, so they can be configured to automatically update based on the latest changes to the Markdown file using a [schedule](https://www.palantir.com/docs/foundry/data-integration/schedules/). This ensures that your analyses and visualizations are up to date and built with a reproducible, customizable workflow that integrates with the rest of the Palantir platform.

![Image 3: The Reports side panel in Code Workspaces.](https://www.palantir.com/docs/resources/foundry/code-workspaces/rstudio-reports.png)
...
1.   Create the Markdown file (`.Rmd` or `.qmd`) you wish to use to create a report. This Markdown file can read from Foundry datasets that have been added to your workspace using the [data side panel](https://www.palantir.com/docs/foundry/code-workspaces/data/#read-data).
...
![Image 4: Previewing a Markdown report in a new tab.](https://www.palantir.com/docs/resources/foundry/code-workspaces/rstudio-reports-preview.png)
...
This feature now includes CRAN library management through Maestro, a Foundry library management tool that seamlessly integrates with conda and CRAN to efficiently handle library installations and maintain managed environments. Newly-installed CRAN libraries will be tracked in your environment files and consistently restored across RStudio® workspaces, Shiny applications, transforms, and model outputs, improving the reproducibility of your environments. See the [get started](https://www.palantir.com/docs/foundry/code-workspaces/rstudio/#get-started) section below to enable this feature.

To convert library install commands to Maestro, refer to the instructions below for either [conda](https://www.palantir.com/docs/foundry/code-workspaces/rstudio/#manage-conda-libraries-with-maestro-recommended) or [CRAN](https://www.palantir.com/docs/foundry/code-workspaces/rstudio/#manage-cran-libraries-with-maestro). With a managed environment, installed libraries in your working environment are tracked through the following files, allowing the environment to be restored to its original state across workflows:

1.   `/home/user/repo/.envs/maestro/meta.yaml`: Known as the environment [**manifest**](https://www.palantir.com/docs/foundry/code-workspaces/jupyterlab/#example-metayaml-file), it contains the set of **requested** libraries and versions for your environment.
2.   `/home/user/repo/.envs/maestro/hawk.lock`: Known as the environment [**lock file**](https://www.palantir.com/docs/foundry/code-workspaces/jupyterlab/#example-hawklock-file), it contains the set of **resolved** libraries and versions for your environment, solved from the constraints specified in the manifest. The `hawk.lock` file replaces `conda.lock`, which has been deprecated.
...
*   Renderings of [Markdown reports](https://www.palantir.com/docs/foundry/code-workspaces/rstudio/#markdown-reports)
...
`1maestro env cran install <library_name>` 
To install a specific version, run the following command:

Copied!

`1maestro env cran install <library_name>@<library_version>` 
*   **Uninstall using the terminal:** To uninstall a library, run the following command in the terminal:

Copied!

`1maestro env uninstall --cran-dependencies <library_names>` 
...
![Image 6: List of installed libraries displayed in the Libraries tab](https://www.palantir.com/docs/resources/foundry/code-workspaces/installed-libraries-tab.png?width=500)
