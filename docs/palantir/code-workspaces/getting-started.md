...
URL Source: https://www.palantir.com/docs/foundry/code-workspaces/getting-started/
...
![Image 1: Select JupyterLab® or RStudio®](https://www.palantir.com/docs/resources/foundry/code-workspaces/launch-code-workspace-1.png)

![Image 2: Name and location selection](https://www.palantir.com/docs/resources/foundry/code-workspaces/launch-code-workspace-2.png)
...
![Image 3: Choose folder and select license](https://www.palantir.com/docs/resources/foundry/code-workspaces/launch-code-workspace-3.png)

Each user who launches the workspace will run their own isolated session. If two or more users will be using the same workspace, ensure that all users have `Viewer` permission or higher for the underlying code repository. See above for more information about a code workspace’s [underlying code repository](https://www.palantir.com/docs/foundry/code-workspaces/getting-started/#backing-code-repository).
...
![Image 4: Advanced gear icon](https://www.palantir.com/docs/resources/foundry/code-workspaces/advanced-gear.png?width=350)
...
*   **Auto-shutdown**
    ...
    *   Longer auto-shutdown settings can incur higher costs. We recommend setting a shorter auto-shutdown period to reduce cost, and to rely on code committed to Foundry rather than uncommitted changes which would be restored across sessions (see the [code workspace lifecycle documentation](https://www.palantir.com/docs/foundry/code-workspaces/getting-started/#code-workspace-lifecycle) below).
...
![Image 5: Advanced settings](https://www.palantir.com/docs/resources/foundry/code-workspaces/code-workspace-advanced-settings.png)
...
![Image 6: Add data dropdown](https://www.palantir.com/docs/resources/foundry/code-workspaces/add-data.png)
...
Review [managed Conda/PyPI environments in Jupyter® Notebooks](https://www.palantir.com/docs/foundry/code-workspaces/jupyterlab/#managed-condapypi-environments-in-jupyter-notebooks) for more information.
...
![Image 7: Source control side panel](https://www.palantir.com/docs/resources/foundry/code-workspaces/source-control.png)
...
![Image 8: Branch selector](https://www.palantir.com/docs/resources/foundry/code-workspaces/branch-selection.png?width=500)

In the same way as [branching elsewhere in Foundry](https://www.palantir.com/docs/foundry/data-integration/branching/), the data available in a code workspace comes from the branch you are currently on (with [fallbacks](https://www.palantir.com/docs/foundry/code-repositories/branch-settings/#fallback-branches)). If you write data to an output dataset, the data will be written to your current branch.
...
![Image 9: Share button](https://www.palantir.com/docs/resources/foundry/code-workspaces/share-button.png?width=500)
...
![Image 10: Restart workspace button](https://www.palantir.com/docs/resources/foundry/code-workspaces/restart-button.png?width=500)
...
You can select **Active > Restore checkpoint** to restore the workspace to the state of a previous checkpoint. See the [checkpoints](https://www.palantir.com/docs/foundry/code-workspaces/getting-started/#checkpoints) section below for more information. Code Workspaces will not automatically restore the workspace to a previous checkpoint following a workspace restart.
...
Code Workspaces sessions are automatically shut down when there is no user activity, and can last at most 24 hours (see more about automatic shutdowns in the [advanced settings](https://www.palantir.com/docs/foundry/code-workspaces/getting-started/#advanced-settings) documentation).

When a session shuts down, all the relevant state is persisted in Foundry in a [checkpoint](https://www.palantir.com/docs/foundry/code-workspaces/getting-started/#checkpoints) and restored the next time the same user opens the code workspace. This allows users to pick up where they were at the time of shutdown.
...
By default, Code Workspaces saves a data checkpoint of a given user's session every ten minutes. Data checkpoints are not shared across users. Code Workspaces also creates a data checkpoint before the workspace shuts down (through a [manual restart](https://www.palantir.com/docs/foundry/code-workspaces/getting-started/#restart-a-code-workspace), a manual shutdown, or an automatic shutdown, for example). An inactive code workspace does not generate any new data checkpoints, and any given data checkpoint lasts up to a maximum of 30 days. As a result, since no valid data checkpoints will be available, a workspace left untouched for more than 30 days will only be able to restore work from what was synced to the backing repository or from a [code checkpoint](https://www.palantir.com/docs/foundry/code-workspaces/getting-started/#code-checkpoints). Data checkpoints are designed to be a safety net to persist the workspaces's state across sessions; they should not be relied on as a primary form of saving. We strongly recommend frequently syncing existing work in the workspace.
...
If you want data to be available to published dashboards or transforms, or to other users, we recommend that you [persist data to a Foundry dataset](https://www.palantir.com/docs/foundry/code-workspaces/data/#write-data) instead of saving it in your workspace.
...
![Image 11: The Data file management dialog is displayed, offering instructions on the file relocation process.](https://www.palantir.com/docs/resources/foundry/code-workspaces/data-file-management.png)
