...
URL Source: https://www.palantir.com/docs/foundry/custom-widgets/dark-theme/

Markdown Content:

# [](https://www.palantir.com/docs/foundry/custom-widgets/dark-theme/#dark-theme-support)Dark theme support
...
2    const isDarkTheme = useDarkTheme();
3    return <Theme appearance={isDarkTheme ? "dark" : "light"}>...</Theme>;
4};
5
6const useDarkTheme = () => useMediaQuery("(prefers-color-scheme: dark)");
7
8const useMediaQuery = (query: string) => {
9    const [matches, setMatches] = useState(false);
10
11    const handleChange = (e: MediaQueryListEvent) => setMatches(e.matches);
12
13    useEffect(() => {
14        const mediaQueryList = window.matchMedia(query);
15        setMatches(mediaQueryList.matches);
16        mediaQueryList.addEventListener("change", handleChange);
17        return () => {
18            mediaQueryList.removeEventListener("change", handleChange);
19        };
20    }, [query]);
21
22    return matches;
...
[← PREVIOUS Add widget set to a Marketplace product](https://www.palantir.com/docs/foundry/custom-widgets/marketplace/)

[NEXT Auto sizing →](https://www.palantir.com/docs/foundry/custom-widgets/auto-sizing/)
