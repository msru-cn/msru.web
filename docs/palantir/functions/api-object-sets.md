...
URL Source: https://www.palantir.com/docs/foundry/functions/api-object-sets/
...
Object sets are more efficient than object arrays for function inputs because they defer loading until needed. For best practices on using object sets efficiently, see [Optimize performance](https://www.palantir.com/docs/foundry/functions/optimize-performance/).
...
*   Array properties support the `.contains()` filter, which filters to objects whose array property values contain _any_ of the given values.
...
```
1import { Filters } from "@foundry/functions-api";
2
3
4Objects.search()
5    .flights()
...
Based on the object type of your object set, _Search Around_ methods are generated to enable traversing [links](https://www.palantir.com/docs/foundry/object-link-types/link-types-overview/) based on the object type of your object set. In the below example, we filter to an object set of Flights based on the departure code, then Search Around to the passengers on those flights. This results in an object set of Passengers, which can be further filtered or searched around on.

Copied!

```
1const passengersDepartingFromAirport = Objects.search()
2    .flights()
3    .filter(flight => flight.departureAirportCode.exactMatch(airportCode))
4    .searchAroundPassengers();
```

Search Around methods will only be generated for link types that are imported into your project. Refer to [the tutorial](https://www.palantir.com/docs/foundry/functions/foo-getting-started/#import-ontology-types) for details on how to import link types.
...
```
1import { Objects } from "@foundry/ontology-api";
2
...
For an example of a full semantic search workflow, review the [semantic search workflow guide](https://www.palantir.com/docs/foundry/ontology/using-palantir-provided-models-to-create-a-semantic-search-workflow/).
...
[Learn more about the structure of these aggregation types, including **valid bucketing types**.](https://www.palantir.com/docs/foundry/functions/types-reference/#aggregation-types)
...
```
1import { Function, ThreeDimensionalAggregation } from "@foundry/functions-api";
2import { Objects } from "@foundry/ontology-api";
3
...
For an example of manipulating aggregation results in memory, try the guide for [creating custom aggregations](https://www.palantir.com/docs/foundry/functions/create-custom-aggregation/).
