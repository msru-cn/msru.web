...
URL Source: https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/

Published Time: Thu, 09 Jul 2026 17:47:51 GMT

# [](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#custom-jdbc-sources)Custom JDBC sources

Connect Foundry to JDBC sources to read and sync data between Foundry and most relational databases and data warehouses.

Once the source is configured, you can flexibly define how data should be synced from the database using a SQL query specified in the extract definition. In addition, you can [configure an incremental sync](https://www.palantir.com/docs/foundry/data-connection/optimize-jdbc-syncs/#incremental-syncs) that only reads data that has been updated since the last sync was run.

## [](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#setup)Setup
...
## [](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#configuration-options)Configuration options
...
| `Driver class` | Yes | Enter the driver class to use when connecting to the database. This driver must be on the classpath at Data Connection runtime. Learn more about [JDBC drivers](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#jdbc-drivers) below. |
| `Credentials` | Yes | The username and password credentials of the JDBC connection. |
| `JDBC properties` | No | Add property names and values to configure connection behavior. Learn more about [JDBC properties](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#jdbc-properties) below. |
| `Advanced Options` | No | Expand this field to add optional JDBC output configurations. Learn more about [output settings](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#output-overrides-source) below. |

### [](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#jdbc-drivers)JDBC drivers
...
#### [](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#foundry-worker-connections)Foundry worker connections

[Foundry worker connections](https://www.palantir.com/docs/foundry/data-connection/core-concepts/#foundry-worker) do not require Palantir-signed drivers. You can typically find and download drivers from the documentation of your particular source type, such as [Google BigQuery ↗](https://cloud.google.com/bigquery/docs/reference/odbc-jdbc-drivers). You can then upload the driver when you [configure the connection](https://www.palantir.com/docs/foundry/data-connection/set-up-source/).

#### [](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#agent-worker-connections-legacy)Agent worker connections [Legacy]

Legacy

Agent worker is in the [legacy](https://www.palantir.com/docs/foundry/platform-overview/development-life-cycle/) phase of development. The configuration below is provided for existing agent worker sources. See [Foundry worker vs agent worker](https://www.palantir.com/docs/foundry/data-connection/foundry-worker-vs-agent-worker/).

For [agent worker](https://www.palantir.com/docs/foundry/data-connection/core-concepts/#agent-worker) sources, drivers must be manually uploaded on the agent.

JDBC drivers uploaded to the agent may cause classpath conflicts with other JDBC drivers already present on the agent. To avoid potential conflicts, use a [Palantir-provided connector](https://www.palantir.com/docs/foundry/data-integration/foundry-provided-drivers/) instead of manually uploading a JDBC driver.
...
![Image 8: Add JDBC drivers to an agent in Data Connection.](https://www.palantir.com/docs/resources/foundry/available-connectors/add-jdbc-driver-to-agent.png)
...
![Image 9: Upload a new JDBC driver in Data Connection.](https://www.palantir.com/docs/resources/foundry/available-connectors/upload-new-jdbc-driver.png)

1.   Select **⊕** to add the driver to the agent and exit the sidebar.

When [configuring a connection to a JDBC source](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#configuration-options) you can only specify the driver class, not the specific driver. For this reason, you should only add one driver of a given class to an agent.

1.   After selecting or uploading the needed driver, you must select **Restart agent** to use the new driver file.

### [](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#jdbc-properties)JDBC properties
...
### [](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#output-overrides-source)Output overrides: Source
...
| `Fetch size` | No | None | The number of rows fetched with each database round trip for a query. Learn more in the [section below](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#fetch-size). |
| `Max file size` | No | None | Specify the maximum size (in bytes or rows) of the output files. Learn more in the [section below](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#maximum-file-size). |

#### [](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#fetch-size)Fetch size
...
#### [](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#maximum-file-size)Maximum file size
...
## [](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#sync-data-from-jdbc-sources)Sync data from JDBC sources
...
Learn more about [source exploration](https://www.palantir.com/docs/foundry/data-connection/source-exploration/) in Foundry.

## [](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#configure-jdbc-syncs)Configure JDBC syncs

### [](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#pre-queries)Pre-queries
...
### [](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#sql-queries)SQL queries
...
![Image 10: The SLQ Query input on the Edit syncs page for a JDBC connector.](https://www.palantir.com/docs/resources/foundry/available-connectors/jdbc-sql-query.png)

### [](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#output-overrides-sync)Output overrides: Sync

Aside from configuring output overrides at the source configuration level, you can choose to apply specific overrides to individual sync outputs. The saved configuration will apply only to the individual sync. Review the [output overrides](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#output-overrides-source) section above for more information about configuration options.

#### [](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#precision-limits)Precision limits
...
## [](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#optimize-jdbc-syncs)Optimize JDBC syncs
...
### [](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#incremental-jdbc-syncs)Incremental JDBC syncs
...
![Image 11: The Incremental configuration panel in the Edit syncs page](https://www.palantir.com/docs/resources/foundry/available-connectors/jdbc-incremental-config.png)
...
### [](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#parallelize-the-sql-query)Parallelize the SQL query
...
![Image 12: The parallelism example explained above, entered into the JDBC sync configuration interface in Data Connection. ](https://www.palantir.com/docs/resources/foundry/available-connectors/jdbc-parallelize-sql-query.png)
...
## [](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#iceberg-syncs-beta)Iceberg syncs [Beta]
...
You can sync data from a Custom JDBC source directly into Foundry [Iceberg tables](https://www.palantir.com/docs/foundry/iceberg/overview/). Iceberg syncs support both non-incremental and incremental batch ingestion, along with multiple write modes, and run on the [Foundry worker](https://www.palantir.com/docs/foundry/data-connection/core-concepts/#foundry-worker) runtime. For configuration details and supported write modes, review the [Iceberg syncs documentation](https://www.palantir.com/docs/foundry/iceberg/syncs/).

## [](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#export-data-to-jdbc-sources)Export data to JDBC sources

Data Connection supports table exports of datasets with schemas to JDBC sources.

Review our documentation to learn how to [enable, configure, and schedule JDBC exports](https://www.palantir.com/docs/foundry/data-connection/export-overview/#table-exports).

### [](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#export-configuration-options-for-jdbc-sources)Export configuration options for JDBC sources
...
![Image 13: A zoomed-in view of the input form for the names of the destination table, schema, and dataset](https://www.palantir.com/docs/resources/foundry/available-connectors/table-export-schema-name-form.png)

![Image 14: A modal appears that allows users to see a source preview and select a destination without having to manually enter its name](https://www.palantir.com/docs/resources/foundry/available-connectors/file-select-preview.png)

#### [](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#export-mode)Export mode

You must select an [export mode](https://www.palantir.com/docs/foundry/data-connection/export-overview/#table-export-modes).

#### [](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#batch-size)Batch size
...
## [](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#health-checks)Health checks
...
## [](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#export-tasks-legacy)Export tasks (legacy)

[Export tasks](https://www.palantir.com/docs/foundry/data-connection/export-tasks/) are a legacy feature that is _not_ recommended for new implementations. All new exports should use the current [recommended export workflow](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#export-data-to-jdbc-sources). This documentation is provided for users who are still using legacy export tasks.

### [](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#supported-databases)Supported databases
...
### [](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#task-configuration)Task configuration
...
```yaml
1type: export-jdbc-task
2exporterType: parquet
3datasets:
4  - inputAlias: myInput  # only required if you have more than 1 input
5    writeDirectly: true
6    writeMode: Overwrite
7    table:
8      database: mydb  # Optional
9      schema: public  # Optional
10      table: mytable
```
...
5parallelize: false  # Run exports for each dataset in parallel (up to 8)
6
7# SQL execution hooks
8preSql:  # SQL statements run before export
9  - TRUNCATE TABLE staging_table
10stagingSql:  # SQL statements run before table rename
11  - SELECT 1
12afterSql:  # SQL statements run after export
13  - INSERT INTO production_table SELECT * FROM staging_table
14
15# Transaction management
16manualTransactionManagement: false  # Set true to manage transactions yourself
17transactionIsolation: READ_COMMITTED  # java.sql.Connection.TransactionIsolationLevel
18
19# Dataset-specific configurations
20datasets:
21  - inputAlias: myInput
22    table:
23      database: mydb
24      schema: public
25      table: mytable
26
27    # Write configuration
28    writeDirectly: false  # false = use staging table first
29    copyMode: insert  # insert or directCopy (PostgreSQL only)
30    batchSize: 1000  # Rows per insert statement
31    writeMode: ErrorIfExists  # ErrorIfExists, Append, Overwrite, AppendIfPossible
32
33    # Incremental export
34    incrementalType: snapshot  # snapshot or incremental
35
36    # Performance
37    exporterThreads: 1  # Parallel file export threads
38
39    # Identifier handling
40    quoteIdentifiers: true  # Quote column names
41
42    # Transaction isolation for this export
43    exportTransactionIsolation: READ_UNCOMMITTED
```

#### [](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#write-modes)Write modes
...
#### [](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#write-directly-vs-staging)Write directly vs. staging
...
#### [](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#copy-modes)Copy modes
...
#### [](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#quote-identifiers)Quote identifiers
...
### [](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#incremental-exports)Incremental exports
...
1type: export-jdbc-task
2preSql:
3  - TRUNCATE TABLE mytable_incremental
4datasets:
5  - inputAlias: myInput
6    table:
7      schema: public
8      table: mytable_incremental
9    writeDirectly: true
10    writeMode: AppendIfPossible
11    incrementalType: incremental
12afterSql:
13  - INSERT INTO mytable SELECT * FROM mytable_incremental
```

### [](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#database-specific-considerations)Database-specific considerations

#### [](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#teradata)Teradata
...
5      type: 'teradata'
6      columnVarcharSize:
7        someColumnName: 1000
```

#### [](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#snowflake)Snowflake
...
#### [](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#oracle)Oracle

When exporting to an Oracle database, you can adapt the maximum column size (default 255 bytes) using `columnVarcharSize`:
...
### [](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#performance-optimization)Performance optimization
...
### [](https://www.palantir.com/docs/foundry/available-connectors/custom-jdbc-sources/#troubleshooting)Troubleshooting
...
[← PREVIOUS Instagram](https://www.palantir.com/docs/foundry/available-connectors/instagram/)

[NEXT Jira Service Management →](https://www.palantir.com/docs/foundry/available-connectors/jira-service-management/)
