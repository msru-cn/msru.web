...
URL Source: https://www.palantir.com/docs/foundry/available-connectors/salesforce/

Markdown Content:

# [](https://www.palantir.com/docs/foundry/available-connectors/salesforce/#salesforce)Salesforce

Foundry's **Salesforce** connector allows you to sync data between Salesforce and Foundry datasets.

This document refers to the latest version of the Salesforce connector. If you are editing an existing Salesforce connector, it may be a legacy version. Review the section on [migration](https://www.palantir.com/docs/foundry/available-connectors/salesforce/#migration) below for more information.

## [](https://www.palantir.com/docs/foundry/available-connectors/salesforce/#supported-capabilities)Supported capabilities
...
## [](https://www.palantir.com/docs/foundry/available-connectors/salesforce/#data-model)Data model
...
### [](https://www.palantir.com/docs/foundry/available-connectors/salesforce/#data-type-mapping)Data type mapping

The connector maps Salesforce API types to the following [Foundry field types](https://www.palantir.com/docs/foundry/data-integration/datasets/#supported-field-types):
...
## [](https://www.palantir.com/docs/foundry/available-connectors/salesforce/#performance-and-limitations)Performance and limitations
...
## [](https://www.palantir.com/docs/foundry/available-connectors/salesforce/#setup)Setup
...
## [](https://www.palantir.com/docs/foundry/available-connectors/salesforce/#authentication)Authentication

Select a credential method to authenticate your Salesforce connection: JWT token or username-password.

### [](https://www.palantir.com/docs/foundry/available-connectors/salesforce/#jwt-token)JWT token
...
#### [](https://www.palantir.com/docs/foundry/available-connectors/salesforce/#part-1-generate-a-certificate)Part 1: Generate a certificate
...
Copied!```bash
1openssl genrsa 1024 | openssl pkcs8 -topk8 -inform PEM -out key.pem -nocrypt
2openssl req -new -x509 -key key.pem -out cert.pem -days 3650
```  
2.   Export the certificate to a PFX store, convert to Base64, and copy to clipboard:

Copied!```bash
1openssl pkcs12 -export -in cert.pem -inkey key.pem | openssl base64 | pbcopy
```  

Be sure to save the certificate value in a secure location. You will need to access the value later in the [configuration](https://www.palantir.com/docs/foundry/available-connectors/salesforce/#part-3-authorize-user).

#### [](https://www.palantir.com/docs/foundry/available-connectors/salesforce/#part-2-create-a-connected-app)Part 2: Create a connected app
...
#### [](https://www.palantir.com/docs/foundry/available-connectors/salesforce/#part-3-authorize-user)Part 3: Authorize user
...
| `Base64 PFX certificate` | Yes | Value obtained from [generating a certificate](https://www.palantir.com/docs/foundry/available-connectors/salesforce/#part-1-generate-a-certificate). |
...
| `Consumer key` | Yes | Enter the consumer key available in the settings of the [connected app](https://www.palantir.com/docs/foundry/available-connectors/salesforce/#part-2-create-a-connected-app). |

### [](https://www.palantir.com/docs/foundry/available-connectors/salesforce/#username-password)Username-password
...
The security token is an automatically generated key that must be added to the password to log in to Salesforce from an untrusted network. Salesforce does not allow users to view the security token within the application; instead, you must log in to Salesforce as the _integration user_ and navigate to **My Settings** in the top right corner. Then, navigate to **Personal > Reset My Security Token**.

### [](https://www.palantir.com/docs/foundry/available-connectors/salesforce/#networking)Networking

If a direct connection is running your Salesforce connector, you must add a [network egress policy](https://www.palantir.com/docs/foundry/administration/configure-egress/) to allowlist the connection.
...
### [](https://www.palantir.com/docs/foundry/available-connectors/salesforce/#certificates-and-private-keys)Certificates and private keys

SSL connections validate servers certificates. Normally, SSL validations happen through a certificate chain; by default, both agent and Foundry workers trust most industry-standard certificate chains. If the server to which you are connecting has a self-signed certificate, or if there is TLS interception during the validation, the connector must trust the certificate. Learn more about [using certificates in Data Connection](https://www.palantir.com/docs/foundry/data-connection/set-up-source/#optional-add-certificates).

## [](https://www.palantir.com/docs/foundry/available-connectors/salesforce/#configuration-options)Configuration options
...
| `Connected app credentials` | Yes | `JWT bearer token` | Contains credentials needed to connect to Salesforce. Review the [Authentication](https://www.palantir.com/docs/foundry/available-connectors/salesforce/#authentication) sections above for more information. |
...
## [](https://www.palantir.com/docs/foundry/available-connectors/salesforce/#sync-data-from-salesforce)Sync data from Salesforce

Use the [exploration view](https://www.palantir.com/docs/foundry/data-connection/source-exploration/) to sync tables. Additional configuration options are available when editing a sync.

### [](https://www.palantir.com/docs/foundry/available-connectors/salesforce/#incremental)Incremental

During incremental syncs with the Salesforce connector, new data is synced if the incremental column value is _greater than or equal_ to the previous observed maximum value. This behavior, though required for precision and correctness of synced data, will lead to duplicate rows in the output dataset. Incremental pipelines should always contain a deduplication step.

### [](https://www.palantir.com/docs/foundry/available-connectors/salesforce/#filtering)Filtering
...
## [](https://www.palantir.com/docs/foundry/available-connectors/salesforce/#migration)Migration
...
### [](https://www.palantir.com/docs/foundry/available-connectors/salesforce/#migrate-connector)Migrate connector
...
2.   Configure the authentication to match that of the legacy source. In the legacy source configuration, find the `auth-method` or `authentication-method` block in the custom YAML. 
    *   If `type: oauth2-jwt`, configure the new source with [JWT authentication](https://www.palantir.com/docs/foundry/available-connectors/salesforce/#jwt-token): 
        *   `claim-sub` from the custom YAML becomes `Username` in new connector configuration.
        *   `x509-cert` is exported to PFX, then Base64 encoded and becomes `Certificate`. Learn more about certificates in the [JWT authentication](https://www.palantir.com/docs/foundry/available-connectors/salesforce/#jwt-token) section above.
    ...
    *   If `type: oauth2-username-password`, configure the new source with [username-password authentication](https://www.palantir.com/docs/foundry/available-connectors/salesforce/#username-password): 
        ...
        *   `client-id` and `client-secret` properties on the legacy source are _not_ required by the new connector.
...
### [](https://www.palantir.com/docs/foundry/available-connectors/salesforce/#migrate-syncs)Migrate syncs

After configuring a new Salesforce connector, create syncs for the objects that were previously synced by the legacy connector. Use the [exploration view](https://www.palantir.com/docs/foundry/data-connection/source-exploration/) to bulk sync multiple objects at once and create new datasets.
...
[← PREVIOUS Sage Business Cloud Accounting](https://www.palantir.com/docs/foundry/available-connectors/sage-business-cloud-accounting/)

[NEXT Salesforce Marketing Cloud →](https://www.palantir.com/docs/foundry/available-connectors/salesforce-marketing-cloud/)
