...
URL Source: https://www.palantir.com/docs/foundry/available-connectors/bigquery/
...
*   **GCP instance account:** Refer to the [Google Cloud documentation ↗](https://cloud.google.com/compute/docs/access/create-enable-service-accounts-for-instances#using) for information on how to set up instance-based authentication.
    ...
    *   Note that [virtual tables](https://www.palantir.com/docs/foundry/data-integration/virtual-tables/) do not support GCP instance authentication credentials.
...
*   **Workload Identity Federation (OIDC):** Follow the displayed source system configuration instructions to set up OIDC. Refer to the [Google Cloud Documentation ↗](https://cloud.google.com/iam/docs/workload-identity-federation-with-other-providers) for details on Workload Identity Federation and [our documentation](https://www.palantir.com/docs/foundry/data-connection/oidc/) for details on how OIDC works with Foundry.
...
| `Credentials settings` | Yes | Configure using the [Authentication](https://www.palantir.com/docs/foundry/available-connectors/bigquery/#authentication) guidance shown above. |
...
| `Settings for temporary tables` | No [[1]](https://www.palantir.com/docs/foundry/available-connectors/bigquery/#temporary-tables-ref) | Enable to use [temporary tables](https://www.palantir.com/docs/foundry/available-connectors/bigquery/#temporary-tables). |
| `gRPC Settings` | No | Advanced settings used to configure gRPC channels. |
| `Spark settings` | No [[2]](https://www.palantir.com/docs/foundry/available-connectors/bigquery/#spark-settings-ref) | Enable **Spark-compatible BIGNUMERIC scale and precision** to read unparameterized `BIGNUMERIC` columns in Spark when using [virtual tables](https://www.palantir.com/docs/foundry/available-connectors/bigquery/#virtual-tables). When enabled, unparameterized `BIGNUMERIC` columns will use maximum precision and scale that is compatible with Spark's `DecimalType`. |
...
[1] Temporary tables must be enabled when registering [BigQuery views ↗](https://cloud.google.com/bigquery/docs/views-intro) via [virtual tables](https://www.palantir.com/docs/foundry/data-integration/virtual-tables/).
...
A single sync can import data up to the size available on the disk. For [syncs running on a Foundry worker](https://www.palantir.com/docs/foundry/data-connection/core-concepts/#foundry-worker), this is typically limited to 600 GB. Use incremental syncs to import larger tables.
...
Note that temporary tables must be enabled when using [BigQuery views ↗](https://cloud.google.com/bigquery/docs/views-intro) with [virtual tables](https://www.palantir.com/docs/foundry/data-integration/virtual-tables/).

## Export data to BigQuery

Export to BigQuery is done via [export tasks](https://www.palantir.com/docs/foundry/data-connection/export-tasks/).

The connector can export to BigQuery in two ways:

*   Through **Google Cloud Storage** as an intermediary store (recommended). See the section below on [exporting via Cloud Storage](https://www.palantir.com/docs/foundry/available-connectors/bigquery/#export-via-cloud-storage-recommended) for more information.
...
| `incrementalType` | Yes | SNAPSHOT | The value can either be SNAPSHOT or REQUIRE_INCREMENTAL. **• Export in snapshot mode:** Contents of the destination table will be replaced. **• Export in incremental mode:** Contents will be [appended](https://www.palantir.com/docs/foundry/available-connectors/bigquery/#append-rows-to-output-table) to the existing table. |
...
[Pro-code alternatives](https://www.palantir.com/docs/foundry/data-connection/core-concepts/#use-in-code) can be used to connect to BigQuery sources for more complex scenarios.

The examples below demonstrate how to connect to a BigQuery source using the [Python client for Google BigQuery ↗](https://cloud.google.com/python/docs/reference/bigquery/latest)`google-cloud-bigquery` in an [external transform](https://www.palantir.com/docs/foundry/data-connection/external-transforms/).
...
This section provides additional details around using [virtual tables](https://www.palantir.com/docs/foundry/data-integration/virtual-tables/) with a BigQuery source. This section is not applicable when syncing to Foundry datasets.
...
| Incremental pipelines | 🟢 Generally available: `APPEND` only [[1]](https://www.palantir.com/docs/foundry/available-connectors/bigquery/#bigquery-incremental) |
| Compute pushdown | 🟢 Generally available |

Consult the [virtual tables documentation](https://www.palantir.com/docs/foundry/data-integration/virtual-tables/#supported-foundry-workflows) for details on the supported Foundry workflows where BigQuery tables can be used as inputs or outputs.

[1] To enable incremental support for pipelines backed by BigQuery virtual tables, ensure that [Time Travel ↗](https://cloud.google.com/bigquery/docs/time-travel) is enabled with the appropriate retention period. This functionality relies on [Change History ↗](https://cloud.google.com/bigquery/docs/change-history) and is currently append-only. The `current` and `added` read modes in [Python Transforms](https://www.palantir.com/docs/foundry/transforms-python/incremental-usage/#incrementaltransforminput) are supported. The `_CHANGE_TYPE` and `_CHANGE_TIMESTAMP` columns will be made available in Python Transforms.
...
*   Ensure that bi-directional connectivity and allowlisting is established as described in the [Networking section of this documentation](https://www.palantir.com/docs/foundry/available-connectors/bigquery/#networking).
...
*   [Temporary tables](https://www.palantir.com/docs/foundry/available-connectors/bigquery/#temporary-tables) must be enabled in order to register [BigQuery views ↗](https://cloud.google.com/bigquery/docs/views-intro), and the relevant roles must be added to the [authentication credentials](https://www.palantir.com/docs/foundry/available-connectors/bigquery/#authentication).
*   Configure [Spark settings](https://www.palantir.com/docs/foundry/available-connectors/bigquery/#connection-details) to read unparameterized `BIGNUMERIC` columns in Spark.

See the [Connection details](https://www.palantir.com/docs/foundry/available-connectors/bigquery/#connection-details) and [Temporary tables](https://www.palantir.com/docs/foundry/available-connectors/bigquery/#temporary-tables) section above for more details.
...
When using BigQuery virtual tables registered to the same source as inputs and outputs to a pipeline, it is possible to fully federate compute to BigQuery. This feature is currently available in Python transforms. See the [Python documentation](https://www.palantir.com/docs/foundry/transforms-python/tables-bigquery/) for details on how to push down compute to BigQuery.
...
![Image 1: The gRPC settings setting section in the BigQuery connection settings.](https://www.palantir.com/docs/resources/foundry/available-connectors/grpc_settings_bigquery.png)

### Preview fails to load for a dataset or virtual table based on a BigQuery view

If your preview fails to load and the target BigQuery table is a view, this may be the result of a schema mismatch. Views' schemas are determined by the result of the SQL query defined at the time of creation. Subsequent changes to the underlying table's structure are _not_ automatically applied to the view's schema and may lead to unexpected behavior when trying to consume the view (such as via a sync/virtual table in the Palantir platform.)
