...
URL Source: https://www.palantir.com/docs/foundry/available-connectors/databricks/

Markdown Content:

# [](https://www.palantir.com/docs/foundry/available-connectors/databricks/#databricks)Databricks
...
## [](https://www.palantir.com/docs/foundry/available-connectors/databricks/#supported-capabilities)Supported capabilities
...
## [](https://www.palantir.com/docs/foundry/available-connectors/databricks/#setup)Setup
...
### [](https://www.palantir.com/docs/foundry/available-connectors/databricks/#connection-details)Connection details
...
| `HTTP Path` | Yes |  | The Databricks compute resource’s HTTP Path value. This can be either a: * SQL warehouse (recommended) of form `/sql/<version>/warehouses/<warehouseId>` * Compute cluster of form `sql/protocolv1/o/<workspaceId>/<clusterId>` |
| `Cloud Fetch` | Yes | True | Indicates whether Cloud Fetch should be enabled. Refer to the [Networking](https://www.palantir.com/docs/foundry/available-connectors/databricks/#cloud-fetch) documentation below to ensure suitable network connectivity to the cloud storage locations. |
...
### [](https://www.palantir.com/docs/foundry/available-connectors/databricks/#authentication)Authentication
...
### [](https://www.palantir.com/docs/foundry/available-connectors/databricks/#networking)Networking

For Databricks connections, add the appropriate [egress policies](https://www.palantir.com/docs/foundry/administration/configure-egress/) when setting up the source in the [Data Connection application](https://www.palantir.com/docs/foundry/data-connection/overview/).

Databricks connections typically open a large number of connections at the same time. When using [agent proxy egress policies](https://www.palantir.com/docs/foundry/administration/configure-egress/#agent-proxy-egress-policies), you may exhaust the connection pool on your agent. If you experience connection pool errors, increase the `maxConnections` and `coreConnections` settings in your [agent proxy configuration](https://www.palantir.com/docs/foundry/data-connection/agent-proxy/#connection-pool-configuration).

The Databricks connector requires network access to the `Hostname` provided in the [configuration options](https://www.palantir.com/docs/foundry/available-connectors/databricks/#connection-details) on port 443. This grants access for Foundry to connect to the Databricks workspace and Unity Catalog REST APIs.

#### [](https://www.palantir.com/docs/foundry/available-connectors/databricks/#cloud-fetch)Cloud Fetch
...
#### [](https://www.palantir.com/docs/foundry/available-connectors/databricks/#external-access-to-storage-locations-virtual-tables-only)External access to storage locations (virtual tables only)
...
| Azure storage location [[1]](https://www.palantir.com/docs/foundry/available-connectors/databricks/#azure-storage-location) | `abfss://<container-name>@<account-name>.dfs.core.windows.net/<table-directory>` | `<account-name>.dfs.core.windows.net` `<account-name>.blob.core.windows.net` | `443` |
...
### [](https://www.palantir.com/docs/foundry/available-connectors/databricks/#more-options-ssl-and-hostname-validation)More options: SSL and hostname validation
...
## [](https://www.palantir.com/docs/foundry/available-connectors/databricks/#virtual-tables)Virtual tables
...
2.   Configure [network egress policies](https://www.palantir.com/docs/foundry/administration/configure-egress/) that allow connectivity from Foundry to the table's storage location.
...
Refer to the [official Databricks documentation ↗](https://docs.databricks.com/aws/en/external-access/) for more information on external access and how to determine the storage locations of tables. Refer to the [Networking](https://www.palantir.com/docs/foundry/available-connectors/databricks/#networking) section of this documentation for details on enabling network access to storage locations.

Unity Catalog credential vending is required to use external access. Credential vending is not supported for all table types and table features. For example, views or tables with row filters do not support credential vending. Refer to the [official Databricks documentation ↗](https://docs.databricks.com/aws/en/external-access/credential-vending) for more information on credential vending and the requirements.

If any of the above requirements are not met, connections to Databricks will be made using JDBC. JDBC is the same mechanism used for [syncs](https://www.palantir.com/docs/foundry/data-connection/set-up-sync/). Refer to the [official Databricks documentation ↗](https://docs.databricks.com/aws/en/integrations/jdbc/) for more information on JDBC connectivity to Databricks.
...
| Table outputs | 🟢 Generally available: [Code Repositories](https://www.palantir.com/docs/foundry/code-repositories/overview/), [Pipeline Builder](https://www.palantir.com/docs/foundry/pipeline-builder/overview/) |
| Incremental pipelines | 🟢 Generally available [[2]](https://www.palantir.com/docs/foundry/available-connectors/databricks/#incremental-pipelines) |
| Compute pushdown | 🟢 Generally available: [Python transforms](https://www.palantir.com/docs/foundry/transforms-python/tables-databricks/), [Pipeline Builder](https://www.palantir.com/docs/foundry/building-pipelines/create-external-pipeline-pb/) |

Consult the [virtual tables documentation](https://www.palantir.com/docs/foundry/data-integration/virtual-tables/#supported-foundry-workflows) for details on the supported Foundry workflows where Databricks tables can be used as inputs or outputs. Functionality may vary depending on whether [external access](https://www.palantir.com/docs/foundry/available-connectors/databricks/#external-access) is enabled.
...
### [](https://www.palantir.com/docs/foundry/available-connectors/databricks/#table-format-and-storage-locations)Table format and storage locations
...
### [](https://www.palantir.com/docs/foundry/available-connectors/databricks/#privileges-on-source-credentials)Privileges on source credentials

For full feature support, we recommend providing the following privileges to the [credentials](https://www.palantir.com/docs/foundry/available-connectors/databricks/#authentication) provided for the source connection. These should be applied on either the catalog, schema, or table depending on the desired inheritance model.
...
| Other | `EXTERNAL USE SCHEMA` | Enables external access to storage locations. Refer to [external access](https://www.palantir.com/docs/foundry/available-connectors/databricks/#external-access) for more details. |
...
### [](https://www.palantir.com/docs/foundry/available-connectors/databricks/#source-configuration-requirements)Source configuration requirements
...
*   Ensure that bi-directional connectivity and allowlisting is established as described in the [Networking section of this documentation](https://www.palantir.com/docs/foundry/available-connectors/databricks/#networking), including the recommended networking to storage locations.
...
See the [Connection Details](https://www.palantir.com/docs/foundry/available-connectors/databricks/#connection-details) section above for more details.

## [](https://www.palantir.com/docs/foundry/available-connectors/databricks/#compute-pushdown)Compute pushdown

Foundry offers the ability to push down compute to Databricks when using virtual tables. When using Databricks virtual tables registered to the same source as inputs and outputs to a pipeline, it is possible to fully federate compute to Databricks. See the [Python documentation](https://www.palantir.com/docs/foundry/transforms-python/tables-databricks/) for details on how to push down compute to Databricks in Python Transforms. To push down compute to Databricks in Pipeline Builder, review the [External pipelines documentation.](https://www.palantir.com/docs/foundry/building-pipelines/create-external-pipeline-pb/).

## [](https://www.palantir.com/docs/foundry/available-connectors/databricks/#external-models)External models
...
Refer to the [official Databricks documentation ↗](https://docs.databricks.com/aws/en/machine-learning/manage-model-lifecycle/#train-and-register-unity-catalog-compatible-models) for more information on making models available in Unity Catalog, and to the [guide](https://www.palantir.com/docs/foundry/integrate-models/external-model-connection-databricks/) on setting up Databricks external models in Foundry.

[← PREVIOUS Couchbase](https://www.palantir.com/docs/foundry/available-connectors/couchbase/)

[NEXT Db2 →](https://www.palantir.com/docs/foundry/available-connectors/db2/)
