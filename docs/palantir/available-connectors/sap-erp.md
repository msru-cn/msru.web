...
URL Source: https://www.palantir.com/docs/foundry/available-connectors/sap-erp/
...
| --- | --- |
| [Setup](https://www.palantir.com/docs/foundry/available-connectors/sap-erp/#setup) | Creating the source and configuring the connection type. |
| [Authentication](https://www.palantir.com/docs/foundry/available-connectors/sap-erp/#authentication) | Supported authentication methods. |
| [Networking and connectivity](https://www.palantir.com/docs/foundry/available-connectors/sap-erp/#networking-and-connectivity) | Egress policies and certificates. |
| [Batch syncs](https://www.palantir.com/docs/foundry/available-connectors/sap-erp/#batch-syncs) | SAP object types, incremental syncs, and sync parameters. |
| [Media sets](https://www.palantir.com/docs/foundry/available-connectors/sap-erp/#media-sets) | Ingesting media files from SAP's Document Management System. |
| [Webhooks](https://www.palantir.com/docs/foundry/available-connectors/sap-erp/#webhooks) | Writing data back to SAP via BAPIs. |
| [Use in code repositories](https://www.palantir.com/docs/foundry/available-connectors/sap-erp/#use-in-code-repositories) | Calling the SAP add-on REST API directly from external transforms or functions. |
...
| [Use in code repositories](https://www.palantir.com/docs/foundry/data-connection/external-transforms/) | 🟢 Generally available |
...
The connection type determines which [SAP object types](https://www.palantir.com/docs/foundry/available-connectors/sap-erp/#sap-object-types) are available. Learn more about [remote connections](https://www.palantir.com/docs/foundry/sap/architecture/#connecting-to-a-remote-sap-erp-system-via-a-gateway) and [remote agent configuration](https://www.palantir.com/docs/foundry/sap/install-sap-remote-agent/).
...
| --- | --- |
| **Basic Auth** | Provide the username and password of the technical user created when installing the [Connector](https://www.palantir.com/docs/foundry/sap/install-sap-connector/). |
...
To reach this SAP system through [SAP Data Accelerator](https://www.palantir.com/docs/foundry/sap/data-accelerator/) (SAP's managed connectivity service for systems exposed through an SAP Cloud Connector), set the source's **host** and **port** to your Cloud Connector virtual host and port. Then, assign the client certificate provided by Palantir, allow egress to the SAP Data Accelerator control plane and data plane, and add the **SAP Data Accelerator** property with your control plane URL. This path requires using a [Foundry worker](https://www.palantir.com/docs/foundry/data-connection/core-concepts/#foundry-worker).
...
3.   Set **Transaction type** to one of the following: 
    *   **APPEND** — for incremental updates. See [Incremental syncs](https://www.palantir.com/docs/foundry/available-connectors/sap-erp/#incremental-syncs) for details.
    *   **SNAPSHOT** — for a full load.

4.   Select an **SAP object type** from the dropdown (see [SAP object types](https://www.palantir.com/docs/foundry/available-connectors/sap-erp/#sap-object-types) below).
...
The available object types depend on the [connection type](https://www.palantir.com/docs/foundry/available-connectors/sap-erp/#connection-type) configured on the source.
...
| **CDS View** | ✓ |  | Extracts data from ABAP CDS (Core Data Services) Views, including views declared with `WITH PARAMETERS`. See [CDS view parameters](https://www.palantir.com/docs/foundry/available-connectors/sap-erp/#cds-view-parameters) below. |
| **HANA View** | ✓ |  | Extracts data from HANA Views enabled in the SAP application layer. See [Ingest HANA views from SAP](https://www.palantir.com/docs/foundry/sap/ingest-hana-views/) for prerequisites. |
| **BW Content Extractor** |  | ✓ | Runs an ERP Business Content extractor. Only appears in the object type dropdown when **Connect via Gateway** is enabled and a **Context** value is set on the source. Use `APPEND` for extractors that support delta extraction. See [Configure extractors](https://www.palantir.com/docs/foundry/sap/configure-extractors/). |
| **Transaction Code** | ✓ | ✓ | Runs an SAP transaction code or ABAP report and ingests the rendered output. See [Transaction Code parameters](https://www.palantir.com/docs/foundry/available-connectors/sap-erp/#transaction-code-parameters) below. |
| **Function Module** | ✓ | ✓ | Calls an SAP function module (such as a BAPI) and ingests the structured response. See [Function module parameters](https://www.palantir.com/docs/foundry/available-connectors/sap-erp/#function-module-parameters) below. |
...
| --- | --- | --- |
| **Filter** | ERP Table, CDS View, HANA View, BW Content Extractor | Refines the data extracted from SAP using a [condition builder](https://www.palantir.com/docs/foundry/available-connectors/sap-erp/#filters). |
...
*   Filter values can also use [dynamic filter](https://www.palantir.com/docs/foundry/available-connectors/sap-erp/#dynamic-filters) keywords and date calculation functions.
...
The SAP ERP connector supports [media sets](https://www.palantir.com/docs/foundry/data-integration/media-sets/) for ingesting media files and documents stored in SAP's Document Management System (DMS).

To create a new media ingest:

1.   Navigate to the source overview page.
2.   Select **+ New** next to **Media set syncs**.
3.   Define your media set format. Providing a specific format will enable you to use transformation steps specific to that format in downstream data transformation. Read more about [media set formats](https://www.palantir.com/docs/foundry/media-sets-advanced-formats/media-overview/#supported-media-set-schemas).
...
![Image 1: Example of source exploration for media sets.](https://www.palantir.com/docs/resources/foundry/available-connectors/explore-media-files.png)
...
![Image 2: Example of a Document Type filter.](https://www.palantir.com/docs/resources/foundry/available-connectors/document-type-filter.png)

## Webhooks

SAP webhooks allow you to write data back to SAP by invoking BAPIs (Business APIs) from Foundry. See [Webhooks](https://www.palantir.com/docs/foundry/data-connection/webhooks-overview/) for an overview of how to set up a webhook.
...
*   For transforms, follow [Set up external transforms](https://www.palantir.com/docs/foundry/data-connection/external-transforms/) and use the `external_systems` decorator.
*   For functions, follow [Make API calls from functions](https://www.palantir.com/docs/foundry/functions/api-calls/) and declare the source in the `@function(sources=[...])` decorator.

### Example: Writeback to SAP via BAPI (transform)

The following [external transform](https://www.palantir.com/docs/foundry/data-connection/external-transforms/) invokes `BAPI_SALESORDER_CHANGE` once per row of an input dataset to update the purchase date on each sales document, and records the SAP `RETURN` table in an output dataset for review.
...
To invoke a BAPI synchronously from a Workshop action, an Ontology action backend, or any other caller of a Foundry function, expose the writeback as a Python [function with a source](https://www.palantir.com/docs/foundry/functions/api-calls/). The function takes typed inputs and returns the SAP `RETURN` structure to the caller.

The source must have [code import enabled and exports enabled](https://www.palantir.com/docs/foundry/functions/api-calls/#configure-access-to-external-apis) for functions to use it. Surface SAP `RETURN` messages with a non-zero severity as [user-facing errors](https://www.palantir.com/docs/foundry/functions/user-facing-error/) so callers can react to them.

### Example: Parallel writeback (transform)

Use the `parallel_writeback` endpoint from an [external transform](https://www.palantir.com/docs/foundry/data-connection/external-transforms/) to send many BAPI calls in a single request. The SAP system spawns multiple background jobs and distributes the calls across them, which is significantly faster than sequential `writeback` calls for large batches.
...
The following [external transform](https://www.palantir.com/docs/foundry/data-connection/external-transforms/) combines the DMS list and download endpoints to fetch documents from SAP's Document Management System and write them to a [media set](https://www.palantir.com/docs/foundry/transforms-python/media-sets/). Use this pattern when the built-in [media set sync](https://www.palantir.com/docs/foundry/available-connectors/sap-erp/#media-sets) does not provide enough control, for example, when document selection depends on Foundry-side data, or when you need to combine multiple filters before downloading.
