...
URL Source: https://www.palantir.com/docs/foundry/available-connectors/ftps/

Markdown Content:

# [](https://www.palantir.com/docs/foundry/available-connectors/ftps/#ftpftps)FTP/FTPS

Connect Foundry to FTP and FTPS servers to sync data between folders and Foundry datasets.

## [](https://www.palantir.com/docs/foundry/available-connectors/ftps/#supported-capabilities)Supported capabilities
...
## [](https://www.palantir.com/docs/foundry/available-connectors/ftps/#data-model)Data model
...
## [](https://www.palantir.com/docs/foundry/available-connectors/ftps/#performance-and-limitations)Performance and limitations

There is no limit to the size of transferable files. However, network issues can result in failures of large-scale transfers. In particular, syncs running on a Foundry worker that take more than two days to run will be interrupted. To avoid network issues, we recommend using smaller file sizes and limiting the number of files that are ingested in every execution of the sync. Syncs can be [scheduled](https://www.palantir.com/docs/foundry/data-connection/set-up-sync/#build-or-schedule-your-batch-sync) to run frequently.

## [](https://www.palantir.com/docs/foundry/available-connectors/ftps/#setup)Setup
...
To access on-premise FTP servers, we recommend you [connect with an agent proxy policy](https://www.palantir.com/docs/foundry/data-connection/architecture/#foundry-worker-with-agent-proxy-policy).

Learn more about [setting up a connector](https://www.palantir.com/docs/foundry/data-connection/set-up-source/) in Foundry.

### [](https://www.palantir.com/docs/foundry/available-connectors/ftps/#authentication)Authentication

FTP/FTPS authentication is completed using username and password. The FTP/FTPS connection may fail if the FTP user does not have permission for the root directory configured for the connection. Refer to [Configuration options](https://www.palantir.com/docs/foundry/available-connectors/ftps/#configuration-options) for more details about the root directory.
...
### [](https://www.palantir.com/docs/foundry/available-connectors/ftps/#networking)Networking

If your FTP/FTPS connection [runs in Foundry](https://www.palantir.com/docs/foundry/data-connection/core-concepts/#foundry-worker), you must add a [network egress policy](https://www.palantir.com/docs/foundry/administration/configure-egress/) to allowlist the connection.

If connecting through a domain name, egress policies should be created for the FTP server domain on both the control port (usually port 21) and the data ports. We recommend creating two network policies: a single port policy for the control port, and a port range policy for the data ports. Data ports are determined by the administrators of the FTP server. If errors continue to occur despite proper egress policy configuration, [file an issue](https://www.palantir.com/docs/foundry/getting-help/issues/) quoting the list of policies applied.
...
### [](https://www.palantir.com/docs/foundry/available-connectors/ftps/#certificates-and-private-keys)Certificates and private keys
...
#### [](https://www.palantir.com/docs/foundry/available-connectors/ftps/#implicitexplicit-ssl)Implicit/explicit SSL
...
### [](https://www.palantir.com/docs/foundry/available-connectors/ftps/#connection-requirements)Connection requirements
...
### [](https://www.palantir.com/docs/foundry/available-connectors/ftps/#active-and-passive-mode)Active and passive mode
...
## [](https://www.palantir.com/docs/foundry/available-connectors/ftps/#configuration-options)Configuration options
...
| `Configure client certificates and private key` | No |  | See [Certifications and private keys](https://www.palantir.com/docs/foundry/available-connectors/ftps/#certificates-and-private-keys) for more information. |
| `Configure server certificates` | No |  | See [Certifications and private keys](https://www.palantir.com/docs/foundry/available-connectors/ftps/#certificates-and-private-keys) for more information. |
...
| `SSL method` | No | EXPLICIT | Whether to use [explicit or implicit SSL](https://www.palantir.com/docs/foundry/available-connectors/ftps/#implicitexplicit-ssl) for FTPS connection. |
...
## [](https://www.palantir.com/docs/foundry/available-connectors/ftps/#sync-data-from-ftp)Sync data from FTP

The FTP connector uses the [file-based sync](https://www.palantir.com/docs/foundry/data-connection/file-based-syncs/) interface.

## [](https://www.palantir.com/docs/foundry/available-connectors/ftps/#troubleshooting)Troubleshooting

### [](https://www.palantir.com/docs/foundry/available-connectors/ftps/#agent-connections)Agent connections
...
### [](https://www.palantir.com/docs/foundry/available-connectors/ftps/#ssl-and-ftps)SSL and FTPS

*   Does your server use a self-signed certificate? Have you added it to the source truststore? See the [SSL and hostname validation](https://www.palantir.com/docs/foundry/available-connectors/ftps/#ssl-and-hostname-validation) section above.
...
[← PREVIOUS Freshdesk](https://www.palantir.com/docs/foundry/available-connectors/freshdesk/)

[NEXT Generic connector →](https://www.palantir.com/docs/foundry/available-connectors/generic/)
