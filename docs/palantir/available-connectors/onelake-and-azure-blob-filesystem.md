...
URL Source: https://www.palantir.com/docs/foundry/available-connectors/onelake-and-azure-blob-filesystem/
...
There is no limit to the size of transferable files. However, network issues can result in failures of large-scale transfers. In particular, syncs [running on a Foundry worker](https://www.palantir.com/docs/foundry/data-connection/core-concepts/#foundry-worker) that take more than two days to run will be interrupted. To avoid network issues, we recommend using smaller file sizes and limiting the number of files that are ingested in every execution of the sync. Syncs can be [scheduled](https://www.palantir.com/docs/foundry/data-connection/set-up-sync/#build-or-schedule-your-batch-sync) to run frequently.
...
*   [Client credentials](https://www.palantir.com/docs/foundry/available-connectors/onelake-and-azure-blob-filesystem/#client-credentials-recommended-for-foundry-direct-connections) (recommended)
*   [Shared access signature](https://www.palantir.com/docs/foundry/available-connectors/onelake-and-azure-blob-filesystem/#shared-access-signature)
*   [Username and password](https://www.palantir.com/docs/foundry/available-connectors/onelake-and-azure-blob-filesystem/#usernamepassword)
*   [Refresh token](https://www.palantir.com/docs/foundry/available-connectors/onelake-and-azure-blob-filesystem/#refresh-token)
*   [Shared key](https://www.palantir.com/docs/foundry/available-connectors/onelake-and-azure-blob-filesystem/#shared-key)
*   [Workload Identity Federation](https://www.palantir.com/docs/foundry/available-connectors/onelake-and-azure-blob-filesystem/#workload-identity-federation-oidc)
*   [Azure managed identity](https://www.palantir.com/docs/foundry/available-connectors/onelake-and-azure-blob-filesystem/#azure-managed-identity-legacy--agent-worker-connections) (legacy — for existing agent worker sources)
...
| Option | Required? | Description |
| --- | --- | --- |
| `Blob SAS token` | Yes | The [shared access signature token](https://www.palantir.com/docs/foundry/available-connectors/onelake-and-azure-blob-filesystem/#shared-access-signature-token). |
...
The Azure [workload identity federation ↗](https://learn.microsoft.com/en-us/entra/workload-id/workload-identity-federation-create-trust-user-assigned-managed-identity) authorization method follows the OpenID Connect (OIDC) protocol. Follow the displayed source system configuration instructions to set up workload identity federation. Review [our documentation](https://www.palantir.com/docs/foundry/data-connection/oidc/) for details on how OIDC works with Foundry.
...
[Azure managed identities ↗](https://docs.microsoft.com/azure/active-directory/managed-identities-azure-resources/overview) do not require credentials to be stored in Foundry. The managed identity used for connecting to data must have appropriate permissions on the underlying storage.
...
| `Catalog` | No | Configure a catalog for tables stored in this storage container. See [Virtual tables](https://www.palantir.com/docs/foundry/available-connectors/onelake-and-azure-blob-filesystem/#virtual-tables) for more details. |
...
The ABFS connector uses the [file-based sync interface](https://www.palantir.com/docs/foundry/data-connection/file-based-syncs/).

## Export data to Azure Data Lake/Blob Storage

To export to ABFS, first [enable exports](https://www.palantir.com/docs/foundry/data-connection/export-overview/#enable-exports-for-source) for your ABFS connector. Then, [create a new export](https://www.palantir.com/docs/foundry/data-connection/export-overview/#create-a-new-export).

### Export tasks (legacy)

To begin exporting data, you must configure an [export task](https://www.palantir.com/docs/foundry/data-connection/export-tasks/). Navigate to the Project folder that contains the connector to which you want to export. Right select on the connector name, then select `Create Data Connection Task`.
...
| `rewritePaths` | No | See [section below](https://www.palantir.com/docs/foundry/available-connectors/onelake-and-azure-blob-filesystem/#rewritepaths) for more information. |
...
| `flagFile` | No | See [section below](https://www.palantir.com/docs/foundry/available-connectors/onelake-and-azure-blob-filesystem/#flag-file) for more information. |
...
This section provides additional details around using [virtual tables](https://www.palantir.com/docs/foundry/data-integration/virtual-tables/) from an Azure Data Lake Storage Gen 2 (Azure Blob Storage) source. This section is not applicable when syncing to Foundry datasets.
...
| Incremental pipelines | 🟢 Generally available for Delta tables: `APPEND` only ([details](https://www.palantir.com/docs/foundry/available-connectors/onelake-and-azure-blob-filesystem/#delta)) 🟢 Generally available for Iceberg tables: `APPEND` only ([details](https://www.palantir.com/docs/foundry/available-connectors/onelake-and-azure-blob-filesystem/#iceberg)) 🔴 Not available for Parquet tables |
| Compute pushdown | 🔴 Not available |

Consult the [virtual tables documentation](https://www.palantir.com/docs/foundry/data-integration/virtual-tables/#supported-foundry-workflows) for details on the supported Foundry workflows where tables stored in ADLS Gen2 can be used as inputs or outputs.
...
*   Ensure that bi-directional connectivity and allowlisting is established as described in the [Networking section of this documentation](https://www.palantir.com/docs/foundry/available-connectors/onelake-and-azure-blob-filesystem/#networking).
...
![Image 1: Virtual Tables ABFS Catalog](https://www.palantir.com/docs/resources/foundry/available-connectors/virtual_tables_abfs_catalog.png)
...
For an `Azure Managed Identity`, this message likely means there is an issue with the metadata endpoint used to retrieve a token on the VM. The VM itself may be in a bad state, or the managed identity may not be attached to the VM yet. To debug this, attempt to retrieve a token from the VM by using the cURL command in the [section above](https://www.palantir.com/docs/foundry/available-connectors/onelake-and-azure-blob-filesystem/#confirm-that-a-managed-identity-flow-works-outside-the-data-connection-agent).

### Receiving an HTML 504 gateway timeout page, starting
