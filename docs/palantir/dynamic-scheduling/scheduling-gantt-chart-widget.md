...
URL Source: https://www.palantir.com/docs/foundry/dynamic-scheduling/scheduling-gantt-chart-widget/
...
The Scheduling Gantt Chart is a Workshop widget that renders an interactive Gantt chart for scheduling or resource allocation workflows. Before setting up the widget, ensure you have configured your [dynamic scheduling Ontology](https://www.palantir.com/docs/foundry/dynamic-scheduling/scheduling-ontology-primitives/).
...
*   Choose a [puck style](https://www.palantir.com/docs/foundry/dynamic-scheduling/scheduling-schedule-layer-style/) and customize colors and interactions (allocation behavior, snap behavior) for pucks.

In the example below, the Scheduling Gantt Chart assigns pilots to flights.

![Image 1: Scheduling Gantt chart example: Assigning pilots to flights.](https://www.palantir.com/docs/resources/foundry/dynamic-scheduling/example-use-case.png?width=800)

## Widget layout

The image below provides an overview of the widget layout.

![Image 2: Sample widget layout.](https://www.palantir.com/docs/resources/foundry/dynamic-scheduling/use-case-widget.png?width=1000)
...
![Image 3: Scheduling Gantt Chart widget with subtitles configured.](https://www.palantir.com/docs/resources/foundry/dynamic-scheduling/setup-4.png?width=200)
...
![Image 4: Scheduling Gantt Chart widget grouping configuration.](https://www.palantir.com/docs/resources/foundry/dynamic-scheduling/setup-5.png?width=300)

![Image 5: Scheduling Gantt Chart widget grouping configuration.](https://www.palantir.com/docs/resources/foundry/dynamic-scheduling/setup-6.png?width=300)

*   **Default Sorts:** Allow one or more default sorts to be applied to the ordering of rows in the chart.
*   **Right Click Menu:** See [triggering actions and events on a row](https://www.palantir.com/docs/foundry/dynamic-scheduling/scheduling-row-level-triggering-actions-and-events/) for more information. 
    *   **Action configuration:** Configure actions related to resource object type. These actions can be standard Ontology create, modify, or delete actions, or a custom [FoO-backed action](https://www.palantir.com/docs/foundry/functions/functions-on-objects/).
...
For each schedule layer, you can select a [puck style](https://www.palantir.com/docs/foundry/dynamic-scheduling/scheduling-schedule-layer-style/) to change the visual representation. See [schedule layer (puck) styling](https://www.palantir.com/docs/foundry/dynamic-scheduling/scheduling-schedule-layer-style/) for more information on puck styles, coloring, and properties.
...
The **Drag & Drop** sub-section configures how pucks can be moved within the chart. See the [drag-and-drop documentation](https://www.palantir.com/docs/foundry/dynamic-scheduling/scheduling-drag-and-drop/) for detailed setup instructions.
...
![Image 6: Puck behavior example: Allocate.](https://www.palantir.com/docs/resources/foundry/dynamic-scheduling/allocation-1.gif?width=600)
...
![Image 7: Puck behavior example: Allocate no overlap.](https://www.palantir.com/docs/resources/foundry/dynamic-scheduling/allocate-no-overlap.gif?width=600)
...
![Image 8: Puck behavior example: Snap to previous.](https://www.palantir.com/docs/resources/foundry/dynamic-scheduling/allocation-swap.gif?width=600)
...
![Image 9: Puck behavior example: Swap with downstream.](https://www.palantir.com/docs/resources/foundry/dynamic-scheduling/allocation-downstream.gif?width=600)

    *   **Snap to previous:** Pucks will automatically snap to the end of the closest existing puck on a given row. 
![Image 10: Puck behavior example: Snap to previous.](https://www.palantir.com/docs/resources/foundry/dynamic-scheduling/allocation-swap.gif?width=600)
...
![Image 11: Snap behavior example.](https://www.palantir.com/docs/resources/foundry/dynamic-scheduling/snap-behavior.gif?width=600)
...
*   **Save Handler Action:** The action that is called when a puck is dragged and dropped or resized.
    ...
    *   When configuring a save handler action, you should supply widget provided parameters as the defaults. Refer to the scheduling [drag-and-drop documentation](https://www.palantir.com/docs/foundry/dynamic-scheduling/scheduling-drag-and-drop/) for more information.
...
![Image 12: Non-configured puck formatting.](https://www.palantir.com/docs/resources/foundry/dynamic-scheduling/setup-puck-color.png?width=600)
...
*   **Custom Suggestions:** Select the suggestion function for your schedule object. The function results will be rendered as highlighted areas on the chart when users pick up a puck. This can be used to indicate where users can or should place pucks. When **Enforce Suggestions** is enabled, users can only drop pucks in highlighted regions (hold Shift to override). See [suggestion functions](https://www.palantir.com/docs/foundry/dynamic-scheduling/scheduling-suggestion-functions/) for more information.
*   **Drag Cursor to Create Action:** Enables a drag action across the interface to create a new object for the schedulable object type. The user initiates action by holding Shift and dragging their cursor within the widget. See the [drag to create documentation](https://www.palantir.com/docs/foundry/dynamic-scheduling/scheduling-drag-to-create/) for more information. 
![Image 13: Drag cursor to create action example.](https://www.palantir.com/docs/resources/foundry/dynamic-scheduling/drag-create-action.gif?width=600)
...
Optional validation rules to apply to the schedule layer. See [validation rules](https://www.palantir.com/docs/foundry/dynamic-scheduling/scheduling-validation-rules/) for more information.
...
![Image 14: Display cursor time flag in schedule interface example.](https://www.palantir.com/docs/resources/foundry/dynamic-scheduling/display-cursor-time-flag.png?width=300)
...
![Image 15: Example: Standard unallocated puck size.](https://www.palantir.com/docs/resources/foundry/dynamic-scheduling/standard-unallocated-puck-size.png?width=500)
...
![Image 16: Example: Standardized puck size.](https://www.palantir.com/docs/resources/foundry/dynamic-scheduling/toggled-puck-size.png?width=500)
...
![Image 17: Example: split rows by schedule layer.](https://www.palantir.com/docs/resources/foundry/dynamic-scheduling/split-rows-by-schedule.png?width=600)
...
See [inline metrics](https://www.palantir.com/docs/foundry/dynamic-scheduling/scheduling-inline-metrics/) for more information.
