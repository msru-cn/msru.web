...
URL Source: https://www.palantir.com/docs/foundry/code-workbook/environment-troubleshooting/

Markdown Content:
## Troubleshooting guide

*   [My custom environment initializes slowly](https://www.palantir.com/docs/foundry/code-workbook/environment-troubleshooting/#my-custom-environment-initializes-slowly)
*   [Environment fails to initialize](https://www.palantir.com/docs/foundry/code-workbook/environment-troubleshooting/#environment-fails-to-initialize)
    *   [Code 1 - Environment resolution error](https://www.palantir.com/docs/foundry/code-workbook/environment-troubleshooting/#code-1---environment-resolution-error)
        *   [New Mamba error messages](https://www.palantir.com/docs/foundry/code-workbook/environment-troubleshooting/#new-mamba-error-messages)
        *   [Legacy Mamba error messages](https://www.palantir.com/docs/foundry/code-workbook/environment-troubleshooting/#new-mamba-error-messages)

    *   [Code 137 - Module ran out of memory](https://www.palantir.com/docs/foundry/code-workbook/environment-troubleshooting/#code-137---module-ran-out-of-memory)
    *   [Other Codes](https://www.palantir.com/docs/foundry/code-workbook/environment-troubleshooting/#other-codes)

*   [Default environment initialization speed is inconsistent](https://www.palantir.com/docs/foundry/code-workbook/environment-troubleshooting/#default-environment-initialization-speed-is-inconsistent)
*   [Packages which require both a Conda package and a JAR](https://www.palantir.com/docs/foundry/code-workbook/environment-troubleshooting/#packages-which-require-both-a-conda-package-and-a-jar)
*   [Other issues](https://www.palantir.com/docs/foundry/code-workbook/environment-troubleshooting/#other-issues)

## My custom environment initializes slowly

Slow initialization generally indicates that the environment definition is too large or too complex. Initialization time tends to increase superlinearly with the number of packages in the environment. Additionally, Foundry will often pre-initialize commonly used environments, so if you created a custom environment based on a default profile, the slowness is likely because you are no longer getting a pre-initialized environment and must wait for a new environment to be created. See [Introduction to Environment Creation](https://www.palantir.com/docs/foundry/code-workbook/environment-creation-overview/) for more details about the environment creation process.
...
3.   **Pin versions** - Add version constraints to some of the packages in the Environment Configuration panel, as discussed in [Selecting an Environment](https://www.palantir.com/docs/foundry/code-workbook/environment-overview/#select-a-profile). It is most effective to do this for packages with many possible versions like `python` and for packages with complex dependency graphs like `scipy`. If adding a particular package caused the environment to become slow, try pinning a version for that package. 
    *   _Caveat_: Adding version pins may cause an environment to become unsatisfiable. If the environment fails to initialize, try relaxing the pin. For example, `python=3.6.10` will match only that version, but `python=3.6` will match any of `python 3.6.x`, and so is more likely to be satisfiable.
    *   One option is to wait for your environment (with no pins) to resolve, then check which versions were selected (see [Viewing Resolved Environment](https://www.palantir.com/docs/foundry/code-workbook/environment-view-resolved/)). You can then add pins for those specific versions until initialization performance is sufficiently fast. It is usually optimal to pin versions for packages with many dependencies while leaving all other packages unpinned.
...
![Image 1: environment_error_dialog](https://www.palantir.com/docs/resources/foundry/code-workbook/environment_error_dialog.png)
...
For guidance on understanding those new messages, how to interpret them, and advice on how to remediate them, see [New error messages](https://www.palantir.com/docs/foundry/transforms-python/environment-troubleshooting/#new-mamba-error-messages).

#### Legacy Mamba Error Messages

Find below a list of common error messages from the legacy error messages, before the [new Mamba error messages](https://www.palantir.com/docs/foundry/code-workbook/environment-troubleshooting/#new-mamba-error-messages) were introduced.
...
`1Problem: nothing provides requested A.a.a.a`
...
`1Problem: nothing provides B-b.b.b needed by A-a.a.a:`
...
`1cannot install both X.a.b.c and X.d.e.f`
...
`1CondaService:ReadRepositoryPermissionDenied`
...
`1Problem: package A-a.a.a requires B >=2.2.5,<2.3.0a0, but none of the providers can be installed`

Note that the package `B` could refer to a transitive dependency of `A`, which you have not explicitly listed as a requirement in your environment. See [Resolved Environment](https://www.palantir.com/docs/foundry/code-workbook/environment-view-resolved/) documentation for more information on transitive dependencies.
...
`1o257.loadClass.: java.lang.ClassNotFoundException:<Class>`

JAR Files can be added to a profile's configuration in the **Advanced** section of the Conda Environment configuration tab in Control Panel (see [Configure Code Workbook Profiles](https://www.palantir.com/docs/foundry/administration/configure-code-workbook-profiles/)). Contact your Palantir representative if you need assistance in configuring such packages.
