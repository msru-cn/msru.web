...
URL Source: https://www.palantir.com/docs/foundry/code-workbook/branching-merging/

Markdown Content:

# [](https://www.palantir.com/docs/foundry/code-workbook/branching-merging/#merging-branches)Merging branches

_Merges_ allow you to stage the changes that will be introduced into the parent branch before deciding to copy them over. If there are logical conflicts between your branch and the parent branch, you can resolve conflicts during the merge and rerun your transforms to check that results are correct.

Throughout the rest of this documentation, we refer to the branch that you are attempting to merge as the _source branch_, and the branch you are attempting to merge into as the _target branch_.

## [](https://www.palantir.com/docs/foundry/code-workbook/branching-merging/#previewing-merges)Previewing merges

![Image 6: merge preview](https://www.palantir.com/docs/resources/foundry/code-workbook/branching-merging-preview.png)
...
![Image 7: merge graph view](https://www.palantir.com/docs/resources/foundry/code-workbook/branching-merging-graph.png)

When you run transforms while merging, Code Workbook automatically creates a _merge branch_ on output datasets. This allows the merge to be isolated from both the target branch and the source branch. These merge branches will appear on your dataset in the form `vector-merge-{source}-{target}-{uuid}`.

## [](https://www.palantir.com/docs/foundry/code-workbook/branching-merging/#resolving-merge-conflicts)Resolving merge conflicts
...
![Image 8: merge conflict](https://www.palantir.com/docs/resources/foundry/code-workbook/branching-merging-conflict.png?width=300)
...
![Image 9: merge conflict code](https://www.palantir.com/docs/resources/foundry/code-workbook/branching-merging-conflict-code.png)

After resolving the merge conflicts, the **Merge branch** button will become enabled.

## [](https://www.palantir.com/docs/foundry/code-workbook/branching-merging/#managing-merges)Managing merges
...
## [](https://www.palantir.com/docs/foundry/code-workbook/branching-merging/#completing-a-merge)Completing a merge
...
![Image 10: confirm merge](https://www.palantir.com/docs/resources/foundry/code-workbook/branching-merging-conflict-confirm.png)
...
[← PREVIOUS Getting started](https://www.palantir.com/docs/foundry/code-workbook/branching-getting-started/)

[NEXT Choose imported dataset branch →](https://www.palantir.com/docs/foundry/code-workbook/branching-imported-datasets/)
