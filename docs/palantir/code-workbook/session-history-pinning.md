...
URL Source: https://www.palantir.com/docs/foundry/code-workbook/session-history-pinning/
...
![Image 1: View session history dropdown button](https://www.palantir.com/docs/resources/foundry/code-workbook/workbooks-view-session-history-button.png)
...
*   **moduleLaunchInfo:** Metadata about the properties assigned to the Spark module itself. This includes the following fields: 
    ...
    *   **jarDependencies:** A list of all jar dependencies manually added to the profile in [Control Panel](https://www.palantir.com/docs/foundry/administration/configure-code-workbook-profiles/).
...
![Image 3: Session history compute tab](https://www.palantir.com/docs/resources/foundry/code-workbook/workbooks-compare-session-compute.png)

*   A different profile was used.
*   The profile was customized.
*   Smaller memory settings were employed on the module.

![Image 4: Session history requested environment tab](https://www.palantir.com/docs/resources/foundry/code-workbook/workbooks-compare-session-requested.png)

*   More recent package versions of Python and pandas were requested, amongst others.

![Image 5: Session history resolved environment tab](https://www.palantir.com/docs/resources/foundry/code-workbook/workbooks-compare-session-resolved.png)
...
Session pinning is designed for debugging purposes, and should not be relied on for long-term, production-ready pipelines. For that reason, a pinned session will only affect _interactive_ environments. Builds executed outside of the Code Workbook interface, such as scheduled builds, will not be affected by the pinned session override. We recommend that you restrict session pinning to occasional troubleshooting and experimentation. Instead, use the [session history](https://www.palantir.com/docs/foundry/code-workbook/session-history-pinning/#session-history) feature to understand the differences between various sessions, and edit the environment definition directly.
...
*   Only sessions using [Artifacts Profiles](https://www.palantir.com/docs/foundry/administration/configure-code-workbook-profiles/#artifacts-profiles) can be pinned
...
The [session history](https://www.palantir.com/docs/foundry/code-workbook/session-history-pinning/#session-history), [session comparison](https://www.palantir.com/docs/foundry/code-workbook/session-history-pinning/#compare-sessions), and [session pinning](https://www.palantir.com/docs/foundry/code-workbook/session-history-pinning/#session-pinning) features mentioned above can be instrumental in troubleshooting failing environments. Particularly, they help address failures of previously working environments. Follow these steps to remediate such cases:

1.   Has the environment worked in the past?

    *   If no, refer to the [Environment Troubleshooting Guide](https://www.palantir.com/docs/foundry/code-workbook/environment-troubleshooting/).
    *   If yes but it is now failing, proceed to the next step.

2.   Using the [**Compare sessions**](https://www.palantir.com/docs/foundry/code-workbook/session-history-pinning/#compare-sessions) feature, select the currently failed session with a previously succeeding and observe the differences in the environment.
    ...
    *   If there are no apparent changes in the environment or the steps above did not help, consult the [Environment Troubleshooting Guide](https://www.palantir.com/docs/foundry/code-workbook/environment-troubleshooting/) or contact Palantir Support for further assistance.

3.   (Optional) While troubleshooting during the previous step, use the [**Session pinning**](https://www.palantir.com/docs/foundry/code-workbook/session-history-pinning/#session-pinning) feature to first ensure that the pinned version of the environment works, and to temporarily unblock the functionality of the Workbook while a more permanent solution is found.
