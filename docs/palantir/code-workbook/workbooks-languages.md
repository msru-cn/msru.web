...
URL Source: https://www.palantir.com/docs/foundry/code-workbook/workbooks-languages/
...
To enable a specific language on a Code Workbook profile, see the Conda Environment section of the [Configuring Code Workbook profiles](https://www.palantir.com/docs/foundry/administration/configure-code-workbook-profiles/#conda-environment) documentation. A series of examples for each of the supported languages are provided below, in the respective introductions to [Python](https://www.palantir.com/docs/foundry/code-workbook/workbooks-languages/#introduction-to-python), [R](https://www.palantir.com/docs/foundry/code-workbook/workbooks-languages/#introduction-to-r), and [SQL](https://www.palantir.com/docs/foundry/code-workbook/workbooks-languages/#introduction-to-python).
...
See [Configuring Code Workbook profiles](https://www.palantir.com/docs/foundry/administration/configure-code-workbook-profiles/#conda-environment) for more information.
...
A Python transform is defined as a Python function with any number of inputs, at most one output, and optionally one or more visualizations. By referencing a transform's alias as a function argument, Code Workbook will automatically pass as input of the transform the output of the mentioned alias. For more information about transforms in Code Workbook, consult the [Transforms overview](https://www.palantir.com/docs/foundry/code-workbook/transforms-overview/) documentation.
...
```
1def child(input_spark_dataframe):
2    from pyspark.sql import functions as F
3    return input_spark_dataframe.filter(F.col('A') == 'value').withColumn('new_column', F.lit(2))
```
...
2spark_df = spark.createDataFrame(pandas_df)
3# Convert to pandas
4pandas_df = spark_df.toPandas()
```
...
2def filtering_before_pandas(input_spark_dataframe):
3    # Use PySpark to filter the data
4    filtered_spark_df = input_spark_dataframe.select("name", "age").filter(F.col("age") <= 18)
5
6    # Convert to a pandas df, which collects the data to the driver
7    pandas_df = filtered_spark_df.toPandas()
8
9    # Perform pandas operations
10    mean_age = pandas_df["age"].mean()
11    pandas_df["age_difference_to_mean"] = pandas_df["age"] - mean_age
12
13    # Output the resulting DataFrame
14    return pandas_df
```
...
```
1import pyspark.pandas as p
2
3return p.from_pandas(df).to_spark().coalesce(1)
```
...
An R transform is defined as an R function, with any number of inputs, at most one output, and optionally one or more visualizations. By referencing a transform's alias as a function argument, Code Workbook will automatically pass as input of the transform the output of the mentioned alias. For more information about transforms in Code Workbook, consult the [transforms overview](https://www.palantir.com/docs/foundry/code-workbook/transforms-overview/) documentation.
...
```
1child <- def(r_dataframe) {
2    library(tidyverse)
3    new_df <- r_dataframe %>% dplyr::select(col_A, col_B) %>% dplyr::filter(col_A == true) %>% dplyr::mutate(new_column=1000)
4    return(new_df)
5}
```
...
```
1# Convert from Spark DataFrame to R data.frame
2new_r_df <- SparkR::collect(spark_df)
3
4# Convert from R data.frame to Spark DataFrame
5 spark_df <- SparkR::as.DataFrame(r_df)
```
...
```
1output_dataset <- function(spark_df) {
2    library(tidyverse)
3
4    # Use SparkR to filter the data
5    input_dataset_filtered <- SparkR::select(spark_df, 'column_A', 'column_B')
6
7    # Convert to R data.frame
8    local_df <- SparkR::collect(input_dataset_filtered)
9
10    # Use tidyverse functions to transform your data
11    local_df <- local_df %>% dplyr::filter(column_A == true) %>% dplyr::mutate(new_column = 1000)
12
13    # Output an R data.frame
14    return(local_df)
15}
```
...
```
1SELECT table_b.col_A, table_b.col_B, table_a.*
2FROM table_a
3JOIN table_b ON table_a.col_C == table_B.col_C
```
