...
Interface action submission criteria apply uniformly to all object types that implement the interface. Before you create an interface action, carefully review which users will have permission to create, modify, or delete objects across all object types that implement the interface. [Learn more about establishing control over interface actions](https://www.palantir.com/docs/foundry/action-types/actions-on-interfaces/#limitations-of-interface-action-rules).

## Using action on interface rules

You can use interface action rules whenever the edits can apply to all the object types that implement the interface. In other words, you can use interface action rules only to modify the _interface shared properties_ or to delete objects. For example, if “Feature request” and “Bug” are object types of the “Ticket” interface, you can use a “Create a ticket” action type to create bugs and feature requests, but you cannot create any property types that are specific to bugs or feature requests.

![Image 1: Using action on interface rules](https://www.palantir.com/docs/resources/foundry/action-types/action_on_interface_rules.png)
...
![Image 2: Creating new interface](https://www.palantir.com/docs/resources/foundry/action-types/action_on_interface_new_interface.png)

1.   Add the shared properties that you want to include in the action (if applicable).
2.   Add metadata to describe your action type. Remember that this metadata should apply to all the object types that implement the interface.
3.   Under **Submission criteria**, choose the users that can execute the action (you can apply more complex criteria later on). Remember that these permissions will apply to all object types that implement the interface, as long as the user has permissions to edit them.
4.   Select **Create** to finalize the action type.
...
![Image 3: Create actions on interface](https://www.palantir.com/docs/resources/foundry/action-types/action_on_interface_create_action.png)
...
![Image 4: Action on interface without primary key](https://www.palantir.com/docs/resources/foundry/action-types/action_on_interface_primary_key.png)
...
Note that primary key values _cannot be modified_ by any action type. Therefore, an action will fail on submission if the action tries to modify a primary key property for a selected object type. Always ensure that the action rule does not modify properties that are likely to be used as a primary key by some of the object types that implement the interface.
...
![Image 5: Action on interface modify primary key](https://www.palantir.com/docs/resources/foundry/action-types/action_on_interface_primary_key_modify.png)
